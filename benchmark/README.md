...
`benchmark_resnet_classification.py` trains a torchvision ResNet-18 and a
TensorPlay ResNet-18 on the animal dataset in `test/data`. Both runs use the
same initial Torch state, ImageNet normalization, deterministic epoch order,
...
It then copies the final Torch state into a fresh TensorPlay model and runs
...
preprocessed batches and the TensorPlay/PyTorch throughput ratio is printed.
The default `--device all` run does the complete comparison once on CPU and
once on CUDA. It also compares `torch.compile(backend="inductor", fullgraph=True)`
with `tensorplay.compile(backend="stax", fullgraph=True, strict_native=True)`.
Compiled training follows the same public boundary as Torch: only the model is
compiled; loss, `backward()`, and SGD remain outside. The report records the
first-step compile cost, steady training throughput, and whether the backward
path is TorchInductor/AOTAutograd or TensorPlay eager autograd. If Stax cannot
...
For a wiring/parity smoke test, use one epoch and smaller inputs:
...
`--no-compile` when only eager training/inference parity is needed. Use
