...
def _emit_redispatch(lines, f, variant, dev_src):
    ...
    ret_void = ret == "void"
    if not mutable:
        lines.append(f"    {call};" if ret_void else f"    return {call};")
    else:
        ...
        for m in mutable:
            ...
            if at.is_mutable_tensor_list:
                ...
                lines.append(f"    {m}.unsafeGetTensorImpl()->bump_version();")
    ...
    for f in funcs:
        ...
        for variant in _variant_instances(f):
            ...
            if f.func_name in autocast_ops:
                ...
                lines.append(
                    "        if (__ac_handle && __ac_handle.getKernel(__ac_key) && autocast::is_enabled(__ac_key)) {")
