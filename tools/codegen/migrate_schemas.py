"""Report schema migrations and optionally update the schema file.

For each function, the tool compares argument names, keyword-only markers,
defaults, tuple-return spelling, and mapped C++ ABI.  Entries with a different
ABI are reported so backend work can proceed one operation at a time.
...
def _canonicalize_schema(func: str, extras: dict | None = None) -> str:
    ...
    schema grammar (only broadcast scalars or `[]`); they are moved to the
...
def canonicalize(args, apply):
    ...
    # derivatives.yaml carries its own `- name:` schema entries in the same
...
def main() -> None:
    ...
    print(f"adopted reference schema: {len(adopted)}")
    print(f"ABI mismatch (keep): {len(mismatched)}")
    print(f"not in reference schema (TP-only): {len(missing_aten)}")
    ...
    if args.apply and adopted:
        header = "# Canonical schema spelling generated by tools/codegen/migrate_schemas.py\n"
