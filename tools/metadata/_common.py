...
def get_tensorplay_version() -> str:
    """Resolve the tensorplay version via tools/generate_tensorplay_version.py.
    ...
    spec = importlib.util.spec_from_file_location(
        "generate_tensorplay_version",
        Path(__file__).resolve().parent.parent / "generate_tensorplay_version.py",
    )
    if spec is None or spec.loader is None:
        raise ImportError("Could not load generate_tensorplay_version.py")
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    return mod.get_tensorplay_version()
