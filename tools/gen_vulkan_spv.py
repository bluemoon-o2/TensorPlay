...
def parse_bindings(src_path: Path) -> list[str]:
    """Recover the ordered descriptor signature from the set-0 bindings."""
    ...
    for line in src_path.read_text(encoding="utf-8").splitlines():
        ...
        rest = match.group(2)
        if re.search(r"\bimage[123]D\b", rest):
            descriptor_type = "VK_DESCRIPTOR_TYPE_STORAGE_IMAGE"
        elif re.search(r"\bsampler[123]D\b", rest):
...
def generate(shader_dir: Path, out_dir: Path, glslang: Path) -> None:
    ...
    for template_name, template_spec in spec.items():
        ...
        for variant in template_spec["shader_variants"]:
            ...
            for combo in combos:
                ...
                compiled[shader_name] = (out_spv, parse_bindings(src_path))
    ...
    for stem, src_path in templates.items():
        ...
        compiled[stem] = (out_spv, parse_bindings(src_path))
