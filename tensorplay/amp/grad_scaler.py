...
def _cuda_amp_definitely_not_available() -> bool:
    try:
...
class _MultiDeviceReplicator:
    ...
    Copies are cached per-device.  ``device`` may be a
    :class:`tensorplay.Device` or a ``(type, index)`` key from
    :func:`_device_key`.
    """

    def __init__(self, master_tensor: tensorplay.Tensor) -> None:
        self.master = master_tensor
        self._per_device_tensors: dict[Any, tensorplay.Tensor] = {}

    def get(self, device: Any) -> tensorplay.Tensor:
        retval = self._per_device_tensors.get(device, None)
        if retval is None:
            target = (
                device
                if isinstance(device, tensorplay.Device)
                else tensorplay.Device(*device)
            )
            retval = self.master.to(device=target, non_blocking=True, copy=True)
    ...
    * If no infs/NaNs are found, ``scaler.step(optimizer)`` runs the underlying ``optimizer.step()`` as usual.
    ...
    def __init__(
        ...
        if self._device == "cuda":
            if enabled and _cuda_amp_definitely_not_available():
                warnings.warn(
                    "tensorplay.cuda.amp.GradScaler is enabled, but CUDA is not available.  Disabling.",
                    stacklevel=2,
                )
                self._enabled = False
        ...
        if self._enabled:
            ...
            self._scale: tensorplay.Tensor | None = None
            self._growth_factor = growth_factor
            self._backoff_factor = backoff_factor
            self._growth_interval = growth_interval
            self._init_growth_tracker = 0
            # self._growth_tracker will be lazily initialized during the first call to scale()
            self._growth_tracker: tensorplay.Tensor | None = None
    ...
    def _lazy_init_scale_growth_tracker(self, dev: Any) -> None:
        if self._growth_tracker is not None:
            raise AssertionError("_growth_tracker initialized before _scale")
        self._scale = tensorplay.full(
            (), self._init_scale, dtype=tensorplay.float32, device=dev
        )
        self._growth_tracker = tensorplay.full(
            (), self._init_growth_tracker, dtype=tensorplay.int32, device=dev
        )

    @overload
    def scale(self, outputs: tensorplay.Tensor) -> tensorplay.Tensor: ...

    @overload
    def scale(self, outputs: list[tensorplay.Tensor]) -> list[tensorplay.Tensor]: ...

    @overload
    def scale(
        self, outputs: tuple[tensorplay.Tensor, ...]
    ) -> tuple[tensorplay.Tensor, ...]: ...

    @overload
    def scale(
        self, outputs: Iterable[tensorplay.Tensor]
    ) -> Iterable[tensorplay.Tensor]: ...

    def scale(
        self,
        outputs: tensorplay.Tensor | Iterable[tensorplay.Tensor],
    ) -> tensorplay.Tensor | Iterable[tensorplay.Tensor]:
        """
        ...
        if isinstance(outputs, tensorplay.Tensor):
            if self._scale is None:
        ...
        def apply_scale(val: tensorplay.Tensor | Iterable[tensorplay.Tensor]):
            if isinstance(val, tensorplay.Tensor):
                if len(stash) == 0:
                    if self._scale is None:
                ...
                return val * stash[0].get(_device_key(val.device))
    ...
    def _unscale_grads_(
        self,
        optimizer: tensorplay.optim.Optimizer,
        inv_scale: tensorplay.Tensor,
        found_inf: tensorplay.Tensor,
        allow_fp16: bool,
    ) -> dict[Any, tensorplay.Tensor]:
        ...
        per_device_and_dtype_grads: dict[
            Any, dict[Any, list[tensorplay.Tensor]]
        ] = defaultdict(lambda: defaultdict(list))
        with tensorplay.no_grad():
            for group in optimizer.param_groups:
                for param in group["params"]:
                    if not isinstance(param, tensorplay.Tensor):
                        raise AssertionError(
                            f"expected param to be tensorplay.Tensor, got {type(param).__name__}"
                        )
                    if param.grad is None:
                        continue
                    if (not allow_fp16) and param.grad.dtype == tensorplay.float16:
                        raise ValueError("Attempting to unscale FP16 gradients.")
                    if param.grad.is_sparse:
                        raise NotImplementedError(
                            "GradScaler does not support sparse gradients yet."
                        )
                    to_unscale = param.grad

                    per_device_and_dtype_grads[_device_key(to_unscale.device)][
                        to_unscale.dtype
                    ].append(to_unscale)
    ...
    def unscale_(self, optimizer: tensorplay.optim.Optimizer) -> None:
        """
        Divides ("unscales") the optimizer's gradient tensors by the scale factor.

        :meth:`unscale_` is optional, serving cases where you need to
        modify or inspect gradients between the backward pass(es) and :meth:`step`.
        ...
        inv_scale = (
            tensorplay.full((), 1.0, dtype=tensorplay.float64, device=self._scale.device)
            .div(self._scale.to(tensorplay.float64))
            .to(tensorplay.float32)
        )
        found_inf = tensorplay.full(
            (), 0.0, dtype=tensorplay.float32, device=self._scale.device
        )
    ...
    def step(
        self, optimizer: tensorplay.optim.Optimizer, *args: Any, **kwargs: Any
    ) -> Any:
        ...
        if getattr(optimizer, "_step_supports_amp_scaling", False):
            ...
            # Otherwise, two Tensor attributes of `grad_scale` and `found_inf` are registered on the passed
            # optimizer so that the optimizer can utilize those to skip the parameter updates or unscale
            # gradients before updating parameters.
            kwargs_ = kwargs
            ...
            if has_grad_scaler_kwarg:
                ...
                for t in optimizer_state["found_inf_per_device"].values():
                    moved = t.to(scaler.device, non_blocking=True)
                    found_sum = (
                ...
                optimizer.found_inf = found_inf_combined  # type: ignore[attr-defined]
    ...
    def update(self, new_scale: float | tensorplay.Tensor | None = None) -> None:
        ...
        Args:
            new_scale (float or :class:`tensorplay.Tensor`, optional, default=None):  New scale factor.
        ...
        if new_scale is not None:
            ...
            if isinstance(new_scale, float):
                ...
                reason = (
                    "new_scale should be a float or a 1-element tensor with requires_grad=False."
                )
    ...
    def _get_scale_async(self) -> tensorplay.Tensor | None:
        return self._scale
    ...
    def __getstate__(self) -> dict[str, Any]:
        ...
        if self._enabled:
            ...
            # Pickling _scale and _growth_tracker Tensors directly triggers storage
            # pickling, so instead, we set the unpickled instance up to reinitialize
            # them lazily.
    ...
    def _check_inf_per_device(
        self, optimizer: tensorplay.optim.Optimizer
    ) -> dict[str, Any]:
        _scale, _ = self._check_scale_growth_tracker("_check_inf_per_device")

        dummy_inv_scale = tensorplay.full(
            (), 1.0, dtype=tensorplay.float32, device=_scale.device
        )
        found_inf = tensorplay.full(
            (), 0.0, dtype=tensorplay.float32, device=_scale.device
        )
