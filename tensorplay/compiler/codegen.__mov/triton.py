...
_CODEGEN_VERSION = "m6-2026-08-27-stream-cache"
...
# benches both families and keeps the winner per shape bucket.
_SPLIT_CANDIDATES = (
    (256, 4),
    (512, 4),
    (1024, 8),
...
def _dim_reduction_config(
    ...
    rblock = min(_next_power_of_two(rnumel), 512)
...
# Candidate table for axis-reduction autotuning: (XBLOCK, num_warps, stages).
# RBLOCK stays shape-derived (next_pow2(rnumel), capped); sweeping it mostly
# duplicates the XBLOCK axis at twice the benchmark cost.
_DIM_REDUCTION_CANDIDATES: tuple[tuple[int, int, int], ...] = (
    (16, 4, 2),
    (32, 4, 2),
    (64, 4, 2),
    (128, 4, 3),
    ...
    def generate(
        ...
        *,
        ...
        if dims_reduction:
            ...
            stride = nprog * block
            div = numel_total is not None and numel_total % block == 0
                ...
                if offset == "0":
                    if div:
                        ...
                        inner.append(
                            ...
                            "mask=xindex < xnumel_tail, other=0.0)"
                        )
                    continue
        ...
        if single_reduction:
            ...
            body.append(f"partial = {spec.finalize_call(last + ', axis=0')}")
            body.append("tl.store(ws_ptr + tl.program_id(0), partial)")
            signature = [
                *(f"in_ptr{index}" for index in range(self.input_count)),
                "ws_ptr",
        ...
        if single_reduction:
            ...
            source += (
                "    ws = tp.empty((wsn,), dtype=inputs[0].dtype, "
                "device=inputs[0].device)\n"
            )
...
def _autotune_dims_program(
    ...
    *,
    ...
    def build(config: tuple[int, int, int]):
    ...
    digest_source = (
        ...
        + f"|{repr(example_inputs[0].device)}|{value_dtype}|{epilogue is not None and repr(epilogue) or ''}"
    )
    decision_key = hashlib.sha256(f"dimred|{digest_source}".encode()).hexdigest()[:24]
    ...
    if payload is not None:
        try:
            ...
            cached = (int(record["xblock"]), int(record["warps"]), int(record["stages"]))
