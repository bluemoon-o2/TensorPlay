...
ENV_VAR_PARALLEL_START = "TP_MP_PARALLEL_START"
...
def _wrap(fn, i, args, error_file):
    # Set the parent-death signal where the platform provides it.
...
def start_processes(
    ...
    # The forkserver path can start workers in parallel when explicitly enabled.
    if (
        ...
        # Set env var TP_MP_PARALLEL_START to 0 to disable parallel start
    ...
    def start_process(i):
        logging.info(f"Starting process {i} via {start_method}")

        # We use the file being created (before being written to) as a way to see
        # if the process crashed. If it exists, it means the process crashed and
        # the error file was written to. We
        ...
        tf = tempfile.NamedTemporaryFile(  # noqa: SIM115
            prefix="tensorplay-errorfile-", suffix=".pickle", delete=False
        )
        tf.close()
        os.unlink(tf.name)
...
def spawn(fn, args=(), nprocs=1, join=True, daemon=False, start_method="spawn"):
    ...
    if start_method != "spawn":
        msg = (
            ...
            " tensorplay.multiprocessing.start_processes(...)"
        )
