...
def _empty_with_matching_layout(
    ...
    for idx in fill_order:
        strides[idx] = stride
        stride *= shape[idx]
    return tensorplay.empty_strided(shape, strides, dtype=query.dtype, device=query.device)


def _input_requires_grad(*tensors: tensorplay.Tensor) -> bool:
    """Returns True if any of the tensors requires grad"""
    return any(t.requires_grad for t in tensors)


def _postprocess_flash_output(inpt_tensor: tensorplay.Tensor, og_size: int) -> tensorplay.Tensor:
...
def _validate_sdpa_input(
    ...
    attn_mask: tensorplay.Tensor | None = None,
    dropout_p=0.0,
    is_causal=False,
    scale=None,
    allow_lowp_kv=False,
