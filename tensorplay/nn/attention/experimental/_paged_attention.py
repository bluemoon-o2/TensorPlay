...
"""
This module implements Paged Attention on top of omni_attention.
...
def _cdiv(x: int | float | tensorplay.Tensor, multiple: int | float | tensorplay.Tensor):
    return (x + multiple - 1) // multiple


class PagedAttention:
    """
    PagedAttention supports omni attention inference with a large batch size.
    ...
    def __init__(
        ...
        self.page_table = -tensorplay.ones(
            (max_batch_size, self.n_pages), dtype=tensorplay.int64, device=device
        )

        # capacity: batch_idx -> allocated sequence length
        self.capacity = tensorplay.zeros(max_batch_size, dtype=tensorplay.int64, device=device)
        ...
        self.physical_to_logical = -tensorplay.ones(
            (max_batch_size, n_pages), dtype=tensorplay.int64, device=device
        )

    def reserve(self, batch_idx: tensorplay.Tensor, seq_len: tensorplay.Tensor) -> None:
        """
        ...
        allocated_pages = tensorplay.tensor(
        ...
        # update metadata
        self.physical_to_logical[batch_idx, allocated_pages] = tensorplay.arange(
            start_page_idx.item(),
            end_page_idx.item(),
            device=num_pages_to_allocate.device,
        )
    ...
    def erase(self, batch_idx: tensorplay.Tensor) -> None:
        """
    ...
    def assign(
        self,
        batch_idx: tensorplay.Tensor,
        input_pos: tensorplay.Tensor,
        k_val: tensorplay.Tensor,
        v_val: tensorplay.Tensor,
        k_cache: tensorplay.Tensor,
        v_cache: tensorplay.Tensor,
    ) -> None:
        """
        ...
        physical_block_idx = tensorplay.gather(
            self.page_table[batch_idx], 1, logical_block_idx.to(tensorplay.int64)
        ).to(tensorplay.int32)  # [B, S]
    ...
    def convert_logical_block_mask(
        ...
        batch_idx: tensorplay.Tensor | None = None,
        kv_len: tensorplay.Tensor | None = None,
    ) -> BlockMask:
        """
        ...
        if batch_idx is None:
            batch_idx = tensorplay.arange(B, device=device)
        page_table = self.page_table[batch_idx]

        new_kv_num_blocks = block_mask.kv_num_blocks.clone()

        new_kv_indices = tensorplay.zeros(
            (B, H, ROWS, self.n_pages), dtype=tensorplay.int32, device=device
        )
        new_kv_indices[:, :, :, :MAX_BLOCKS_IN_COL] = (
            tensorplay.gather(
                page_table, 1, block_mask.kv_indices.view(B, -1).to(tensorplay.int64)
            )
        ...
        if block_mask.full_kv_num_blocks is not None:
            ...
            new_full_kv_indices = tensorplay.zeros(
                (B, H, ROWS, self.n_pages), dtype=tensorplay.int32, device=device
            )
            new_full_kv_indices[:, :, :, :MAX_BLOCKS_IN_COL] = (
                tensorplay.gather(
                    page_table,
                    1,
                    block_mask.full_kv_indices.view(B, -1).to(tensorplay.int64),
                )
    ...
    def get_mask_mod(
        ...
        Args:
            ...
            kv_len (Optional[tensorplay.Tensor]): actual KV sequence length for upper bound check.
        """
        if mask_mod is None:
            mask_mod = noop_mask
        ...
        def new_mask_mod(
            b: tensorplay.Tensor,
            h: tensorplay.Tensor,
            q_idx: tensorplay.Tensor,
            physical_kv_idx: tensorplay.Tensor,
        ):
            ...
            return tensorplay.where(is_valid, mask_mod(b, h, q_idx, logical_kv_idx), False)
