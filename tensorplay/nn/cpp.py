...
class OrderedDictWrapper:
    ...
    at the time of access. ``tensorplay.nn.Module`` accesses ``_parameters`` et al. via ``self.__dict__``
    ...
    # Magic methods cannot be assigned dynamically and bypass ``getattr``
    # so we must manually override them.
...
class ModuleWrapper(nn.Module):
    """A subclass of ``tensorplay.nn.Module`` that wraps a C++ frontend module and delegates all access."""
