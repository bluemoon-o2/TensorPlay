...
import tensorplay
from tensorplay import device, dtype, Tensor
from tensorplay.primitives import DeviceLikeType
from tensorplay.nn.parameter import Buffer, Parameter
from tensorplay.utils._dispatch import is_traceable_wrapper_subclass  # noqa
from tensorplay.utils.hooks import BackwardHook, RemovableHandle
...
def register_module_full_backward_pre_hook(
    ...
    registered by :meth:`tensorplay.nn.Module.register_full_backward_pre_hook`.
    Refer to its documentation for more details.

    Hooks registered using this function will be called before hooks registered
    using :meth:`tensorplay.nn.Module.register_full_backward_pre_hook`.
...
def register_module_full_backward_hook(
    ...
    Hooks registered using this function behave in the same way as those
    registered by :meth:`tensorplay.nn.Module.register_full_backward_hook`.
    Refer to its documentation for more details.

    Hooks registered using this function will be called before hooks registered
    using :meth:`tensorplay.nn.Module.register_full_backward_hook`.
...
def _parse_to(*args, **kwargs):
    ...
    if 'memory_format' in kwargs:
        convert_to_format = kwargs['memory_format']

    for arg in args:
        if isinstance(arg, tensorplay.Tensor):
            device = arg.device
            dtype = arg.dtype
        elif isinstance(arg, (tensorplay.Device, str)):
            device = arg
        elif isinstance(arg, tensorplay.DType):
            dtype = arg
...
class Module:
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        # tensorplay._C._log_api_usage_once("python.nn_module")
    ...
    def register_buffer(
        ...
        if "_buffers" not in self.__dict__:
            ...
            raise TypeError(
                f"buffer name should be a string. Got {tensorplay.typename(name)}"
            )
        elif "." in name:
            ...
            isinstance(tensor, tensorplay.Tensor) or hasattr(tensor, "__tensorplay_function__")
        ):
            raise TypeError(
                f"cannot assign '{tensorplay.typename(tensor)}' object to buffer '{name}' "
                "(tensorplay Tensor or None required)"
            )
        else:
    ...
    def register_parameter(self, name: str, param: Optional[Parameter]) -> None:
        ...
        if "_parameters" not in self.__dict__:
            ...
            raise TypeError(
                f"parameter name should be a string. Got {tensorplay.typename(name)}"
            )
        elif "." in name:
        ...
        if param is None:
            ...
            raise TypeError(
                f"cannot assign '{tensorplay.typename(param)}' object to parameter '{name}' "
                "(tensorplay.nn.Parameter or None required)"
            )
        elif param.grad_fn:
            raise ValueError(
    ...
    def add_module(self, name: str, module: Optional["Module"]) -> None:
        ...
        if not isinstance(module, Module) and module is not None:
            raise TypeError(f"{tensorplay.typename(module)} is not a Module subclass")
        elif not isinstance(name, str):
            raise TypeError(
                f"module name should be a string. Got {tensorplay.typename(name)}"
            )
    ...
    def get_submodule(self, target: str) -> "Module":
        ...
        for item in atoms:
            ...
            if not isinstance(mod, tensorplay.nn.Module):
    ...
    def set_submodule(
        ...
        if len(atoms) == 1:
            parent: tensorplay.nn.Module = self
        else:
    ...
    def get_parameter(self, target: str) -> "Parameter":
        ...
        mod: tensorplay.nn.Module = self.get_submodule(module_path)
        ...
        param: tensorplay.nn.Parameter = getattr(mod, param_name)

        if not isinstance(param, tensorplay.nn.Parameter):
    ...
    def get_buffer(self, target: str) -> "Tensor":
        ...
        buffer: tensorplay.Tensor = getattr(mod, buffer_name)
    ...
    def get_extra_state(self) -> Any:
        ...
        raise RuntimeError(
            ...
            "Please file an issue at https://github.com/pytensorplay/pytensorplay/issues/new?template=bug-report.yml "
            "to report this bug."
        )
    ...
    def to_empty(
        ...
        """
        return self._apply(
            lambda t: tensorplay.empty_like(t, device=device), recurse=recurse
        )
    ...
    def to(self, *args, **kwargs):
        ...
        .. function:: to(memory_format=tensorplay.channels_last)
           :noindex:

        Its signature is similar to :meth:`tensorplay.Tensor.to`, but only accepts
        ...
        Args:
            device (:class:`tensorplay.device`): the desired device of the parameters
                and buffers in this module
            dtype (:class:`tensorplay.dtype`): the desired floating point or complex dtype of
                the parameters and buffers in this module
            tensor (tensorplay.Tensor): Tensor whose dtype and device are the desired
                dtype and device for all parameters and buffers in this module
            memory_format (:class:`tensorplay.memory_format`): the desired memory
        ...
        """
        device, dtype, non_blocking, convert_to_format = _parse_to(
            *args, **kwargs
        )
        ...
        def convert(t):
            target_dtype = dtype if t.is_floating_point() or t.is_complex() else None
            try:
                ...
                if str(e) == "Cannot copy out of meta tensor; no data!":
                    raise NotImplementedError(
                        f"{e} Please use tensorplay.nn.Module.to_empty() instead of tensorplay.nn.Module.to() "
    ...
    def register_full_backward_pre_hook(
        ...
        Args:
            ...
            prepend (bool): If true, the provided ``hook`` will be fired before
                ...
                :class:`tensorplay.nn.Module`. Otherwise, the provided
                ``hook`` will be fired after all existing ``backward_pre`` hooks
                on this :class:`tensorplay.nn.Module`. Note that global
    ...
    def register_full_backward_hook(
        ...
        Args:
            ...
            prepend (bool): If true, the provided ``hook`` will be fired before
                ...
                this :class:`tensorplay.nn.Module`. Note that global
    ...
    # tensorplayrec tests the code consistency with the following code
    # fmt: off
    def _call_impl(self, *args, **kwargs):
        forward_call = self.forward # (self._slow_forward if tensorplay._C._get_tracing_state() else self.forward)
        ...
        def inner():
            ...
            if bw_hook:
                if not isinstance(result, (tensorplay.Tensor, tuple)):
    ...
    # on `tensorplay.nn.Module` and all its subclasses is largely disabled as a result. See:
    # https://github.com/pytensorplay/pytensorplay/pull/115074
    ...
    def __setattr__(self, name: str, value: Union[Tensor, "Module"]) -> None:
        ...
        if isinstance(value, Parameter):
            ...
            if value is not None:
                raise TypeError(
                    f"cannot assign '{tensorplay.typename(value)}' as parameter '{name}' "
                    "(tensorplay.nn.Parameter or None expected)"
                )
            self.register_parameter(name, value)
        else:
            ...
            if isinstance(value, Module):
                ...
                if value is not None:
                    raise TypeError(
                        f"cannot assign '{tensorplay.typename(value)}' as child module '{name}' "
                        "(tensorplay.nn.Module or None expected)"
                    )
                for hook in _global_module_registration_hooks.values():
                    output = hook(self, name, value)
                    if output is not None:
                ...
                if isinstance(value, Buffer) or buffers is not None and name in buffers:
                    if value is not None and not (
                        ...
                        or hasattr(value, "__tensorplay_function__")
                    ):
                        raise TypeError(
                            f"cannot assign '{tensorplay.typename(value)}' as buffer '{name}' "
                            "(tensorplay.nn.Buffer, tensorplay.Tensor or None expected)"
                        )
                    ...
                    if (
                        ...
                        is tensorplay.nn.Module.register_buffer
                    ):
                        self.register_buffer(name, value, persistent)
                    else:
    ...
    def _register_state_dict_hook(self, hook):
        r"""Register a post-hook for the :meth:`~tensorplay.nn.Module.state_dict` method.
    ...
    def register_state_dict_post_hook(self, hook):
        ...
        # https://github.com/pytensorplay/pytensorplay/issues/117437 where the return value
    ...
    def register_state_dict_pre_hook(self, hook):
        r"""Register a pre-hook for the :meth:`~tensorplay.nn.Module.state_dict` method.
    ...
    def _save_to_state_dict(self, destination, prefix, keep_vars) -> None:
        ...
        submodule in :meth:`~tensorplay.nn.Module.state_dict`.
    ...
    def state_dict(self, *args, destination=None, prefix="", keep_vars=False):
        ...
        Args:
            ...
            keep_vars (bool, optional): by default the :class:`~tensorplay.Tensor` s
        ...
        if len(args) > 0:
            ...
            warnings.warn(
                ...
                "https://pytensorplay.org/docs/main/generated/tensorplay.nn.Module.html#tensorplay.nn.Module.state_dict"
                " for details.",
                FutureWarning,
                stacklevel=2,
            )
    ...
    def _register_load_state_dict_pre_hook(self, hook, with_module=False):
        r"""See :meth:`~tensorplay.nn.Module.register_load_state_dict_pre_hook` for details.
        ...
        :meth:`~tensorplay.nn.Module.register_load_state_dict_pre_hook` always takes the
    ...
    def _load_from_state_dict(
        ...
        in :meth:`~tensorplay.nn.Module.load_state_dict`. Metadata saved for this
        ...
        .. note::
            ...
            :attr:`state_dict` to :meth:`~tensorplay.nn.Module.load_state_dict`. So
            it can be modified.
        ...
        Args:
            ...
            error_msgs (list of str): error messages should be added to this
                ...
                :meth:`~tensorplay.nn.Module.load_state_dict`
        """
        ...
        use_swap_tensors = tensorplay.__future__.get_swap_module_params_on_conversion()

        for name, param in local_state.items():
            key = prefix + name
            if key in state_dict:
                input_param = state_dict[key]
                if not tensorplay.overrides.is_tensor_like(input_param):
                    error_msgs.append(
                        f'While copying the parameter named "{key}", '
                        "expected tensorplay.Tensor or Tensor-like object from checkpoint but "
                ...
                is_param_lazy = tensorplay.nn.parameter.is_lazy(param)
                ...
                try:
                    with tensorplay.no_grad():
                        if use_swap_tensors:
                            ...
                            if isinstance(param, tensorplay.nn.Parameter):
                                if not isinstance(new_input_param, tensorplay.nn.Parameter):
                                    new_input_param = tensorplay.nn.Parameter(
                                        new_input_param,
                                        requires_grad=param.requires_grad,
                                    )
                                else:
                                    new_input_param.requires_grad_(param.requires_grad)
                            tensorplay.utils.swap_tensors(param, new_input_param)
                            del new_input_param
                        elif assign_to_params_buffers:
                            # Shape checks are already done above
                            if isinstance(param, tensorplay.nn.Parameter):
                                if not isinstance(input_param, tensorplay.nn.Parameter):
                                    input_param = tensorplay.nn.Parameter(
    ...
    def load_state_dict(
        ...
        by this module's :meth:`~tensorplay.nn.Module.state_dict` function.

        .. warning::
            If :attr:`assign` is ``True`` the optimizer must be created after
            the call to :attr:`load_state_dict` unless
            :func:`~tensorplay.__future__.get_swap_module_params_on_conversion` is ``True``.
        ...
        Args:
            ...
            strict (bool, optional): whether to strictly enforce that the keys
                ...
                :meth:`~tensorplay.nn.Module.state_dict` function. Default: ``True``
            assign (bool, optional): When set to ``False``, the properties of the tensors
                in the current module are preserved whereas setting it to ``True`` preserves
                properties of the Tensors in the state dict. The only
                exception is the ``requires_grad`` field of :class:`~tensorplay.nn.Parameter`
    ...
    def eval(self) -> Self:
        ...
        This is equivalent with :meth:`self.train(False) <tensorplay.nn.Module.train>`.
    ...
    def zero_grad(self, set_to_none: bool = True) -> None:
        ...
        See similar function under :class:`tensorplay.optim.Optimizer` for more context.

        Args:
            set_to_none (bool): instead of setting to zero, set the grads to None.
                See :meth:`tensorplay.optim.Optimizer.zero_grad` for details.
        """
    ...
    def share_memory(self) -> Self:
        r"""See :meth:`tensorplay.Tensor.share_memory_`."""
    ...
    def compile(self, *args, **kwargs):
        """
        Compile this Module's forward using :func:`tensorplay.compile`.

        This Module's `__call__` method is compiled and all arguments are passed as-is
        to :func:`tensorplay.compile`.
