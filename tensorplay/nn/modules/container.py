...
class Sequential(Module):
    ...
    :class:`tensorplay.nn.ModuleList`? A ``ModuleList`` is exactly what it
    ...
    def __add__(self, other) -> Sequential:
        if isinstance(other, Sequential):
            ...
            raise ValueError(
                ...
                f"of Sequential class, but {type(other)} is given."
            )
...
class ModuleList(Module):
    ...
    :class:`~tensorplay.nn.ModuleList` can be indexed like a regular Python list, but
    modules it contains are properly registered, and will be visible by all
    :class:`~tensorplay.nn.Module` methods.
...
class ModuleDict(Module):
    ...
    :class:`~tensorplay.nn.ModuleDict` can be indexed like a regular Python dictionary,
    but modules it contains are properly registered, and will be visible by all
    :class:`~tensorplay.nn.Module` methods.

    :class:`~tensorplay.nn.ModuleDict` is an **ordered** dictionary that respects

    * the order of insertion, and

    * in :meth:`~tensorplay.nn.ModuleDict.update`, the order of the merged
      ``OrderedDict``, ``dict`` (started from Python 3.6) or another
      :class:`~tensorplay.nn.ModuleDict` (the argument to
      :meth:`~tensorplay.nn.ModuleDict.update`).

    Note that :meth:`~tensorplay.nn.ModuleDict.update` with other unordered mapping
    ...
    def update(self, modules: Mapping[str, Module]) -> None:
        r"""Update the :class:`~tensorplay.nn.ModuleDict` with key-value pairs from a mapping, overwriting existing keys.

        .. note::
            If :attr:`modules` is an ``OrderedDict``, a :class:`~tensorplay.nn.ModuleDict`, or
            an iterable of key-value pairs, the order of new elements in it is preserved.

        Args:
            modules (iterable): a mapping (dictionary) from string to :class:`~tensorplay.nn.Module`,
                or an iterable of key-value pairs of type (string, :class:`~tensorplay.nn.Module`)
        """
        if not isinstance(modules, container_abcs.Iterable):
            raise TypeError(
...
class ParameterList(Module):
    ...
    :class:`~tensorplay.nn.ParameterList` can be used like a regular Python
    list, but Tensors that are :class:`~tensorplay.nn.Parameter` are properly registered,
    and will be visible by all :class:`~tensorplay.nn.Module` methods.

    Note that the constructor, assigning an element of the list, the
    :meth:`~tensorplay.nn.ParameterList.append` method and the :meth:`~tensorplay.nn.ParameterList.extend`
    method will convert any :class:`~tensorplay.Tensor` into :class:`~tensorplay.nn.Parameter`.
        ...
        class MyModule(nn.Module):
            def __init__(self) -> None:
                ...
                self.params = nn.ParameterList(
                    [nn.Parameter(tensorplay.randn(10, 10)) for i in range(10)]
                )
    ...
    def __setitem__(self, idx: int, param: Any) -> None:
        ...
        idx = self._get_abs_string_index(idx)
        if isinstance(param, tensorplay.Tensor) and not isinstance(param, Parameter):
    ...
    def extend(self, values: Iterable[Any]) -> Self:
        ...
        if not isinstance(values, container_abcs.Iterable) or isinstance(values, tensorplay.Tensor):
            raise TypeError(
    ...
    def extra_repr(self) -> str:
        ...
        for k, p in enumerate(self):
            if isinstance(p, tensorplay.Tensor):
                size_ = "x".join(str(size) for size in p.size())
...
class ParameterDict(Module):
    ...
    :class:`~tensorplay.nn.ParameterDict` is an **ordered** dictionary.
    :meth:`~tensorplay.nn.ParameterDict.update` with other unordered mapping
    types (e.g., Python's plain ``dict``) does not preserve the order of the
    merged mapping. On the other hand, ``OrderedDict`` or another :class:`~tensorplay.nn.ParameterDict`
    will preserve their ordering.

    Note that the constructor, assigning an element of the dictionary and the
    :meth:`~tensorplay.nn.ParameterDict.update` method will convert any :class:`~tensorplay.Tensor` into
    :class:`~tensorplay.nn.Parameter`.
        ...
        class MyModule(nn.Module):
            def __init__(self) -> None:
                ...
                self.params = nn.ParameterDict(
                    {
                        "left": nn.Parameter(tensorplay.randn(5, 10)),
                        "right": nn.Parameter(tensorplay.randn(5, 10)),
                    }
                )
    ...
    def copy(self) -> ParameterDict:
        """Return a copy of this :class:`~tensorplay.nn.ParameterDict` instance."""
    ...
    @staticmethod
    def fromkeys(keys: Iterable[str], default: Optional[Any] = None) -> ParameterDict:
    ...
    def update(self, parameters: Union[Mapping[str, Any], ParameterDict]) -> None:
        r"""Update the :class:`~tensorplay.nn.ParameterDict` with key-value pairs from ``parameters``, overwriting existing keys.

        .. note::
            If :attr:`parameters` is an ``OrderedDict``, a :class:`~tensorplay.nn.ParameterDict`, or
            an iterable of key-value pairs, the order of new elements in it is preserved.

        Args:
            parameters (iterable): a mapping (dictionary) from string to
                :class:`~tensorplay.nn.Parameter`, or an iterable of
                key-value pairs of type (string, :class:`~tensorplay.nn.Parameter`)
        """
