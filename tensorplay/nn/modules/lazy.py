...
class LazyModuleMixin:
    ...
    :class:`tensorplay.nn.UninitializedParameter` s that should not be accessed
    or used, and afterward they contain regular :class:`tensorplay.nn.Parameter` s.
    ...
    typical :class:`tensorplay.nn.Linear`.
    ...
    >>> # xdoctest: +SKIP
    >>> class LazyMLP(tensorplay.nn.Module):
    ...     def __init__(self) -> None:
    ...         super().__init__()
    ...         self.fc1 = tensorplay.nn.LazyLinear(10)
    ...         self.relu1 = tensorplay.nn.ReLU()
    ...         self.fc2 = tensorplay.nn.LazyLinear(1)
    ...         self.relu2 = tensorplay.nn.ReLU()
    ...
    ...
    >>> lazy_mlp(tensorplay.ones(10, 10).cuda())
    ...
    >>> optim = tensorplay.optim.SGD(lazy_mlp.parameters(), lr=0.01)
    ...
    For example, if the LazyMLP class defined above had a :class:`tensorplay.nn.LazyLinear` module
    first and then a regular :class:`tensorplay.nn.Linear` second, the second module would be
    ...
    Lazy modules can load regular :class:`tensorplay.nn.Parameter` s (i.e. you can serialize/deserialize
    initialized LazyModules and they will remain initialized)


    >>> full_mlp = LazyMLP()
    >>> # Dry run to initialize another module
    >>> full_mlp.forward(tensorplay.ones(10, 1))
    ...
    def _lazy_load_hook(
        ...
        module instance.
        See comment in ``tensorplay.nn.Module._register_load_state_dict_pre_hook``
    ...
    def _infer_parameters(self: _LazyProtocol, module, args, kwargs=None):
        ...
        using :class:`tensorplay.nn.parameter.ParameterMode.Infer`, runs a forward pass
