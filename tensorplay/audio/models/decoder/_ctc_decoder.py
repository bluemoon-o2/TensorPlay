...
class CTCHypothesis(NamedTuple):
    ...
    Note:
        ...
        :func:`~tensorplay.audio.models.decoder.CTCDecoder.idxs_to_tokens` instead.
    """

    score: float
    """Score corresponding to hypothesis"""

    timesteps: tensorplay.IntTensor
...
class CTCDecoder:
    ...
    def _get_tokens(self, idxs: tensorplay.IntTensor) -> tensorplay.LongTensor:
        idxs = (g[0] for g in it.groupby(idxs))
        idxs = filter(lambda x: x != self.blank, idxs)
        return tensorplay.LongTensor(list(idxs))

    def _get_timesteps(self, idxs: tensorplay.IntTensor) -> tensorplay.IntTensor:
        ...
        for i, idx in enumerate(idxs):
            ...
            if i == 0 or idx != idxs[i - 1]:
                timesteps.append(i)
        return tensorplay.IntTensor(timesteps)
    ...
    def decode_step(self, emissions: tensorplay.FloatTensor):
        ...
        Args:
            emissions (tensorplay.FloatTensor): CPU tensor of shape `(frame, num_tokens)` storing sequences of
                probability distribution over labels; output of acoustic model.

        Example:
            >>> decoder = tensorplay.audio.models.decoder.ctc_decoder(...)
        ...
        """
        if emissions.dtype != tensorplay.float32:
    ...
    def __call__(
        self, emissions: tensorplay.FloatTensor, lengths: Optional[tensorplay.Tensor] = None
        ...
        Args:
            emissions (tensorplay.FloatTensor): CPU tensor of shape `(batch, frame, num_tokens)` storing sequences of
                probability distribution over labels; output of acoustic model.
    ...
    def idxs_to_tokens(self, idxs: tensorplay.LongTensor) -> List:
        """
