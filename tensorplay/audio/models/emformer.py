...
def _gen_padding_mask(
    ...
    right_context: tensorplay.Tensor,
    summary: tensorplay.Tensor,
    lengths: tensorplay.Tensor,
    mems: tensorplay.Tensor,
    left_context_key: Optional[tensorplay.Tensor] = None,
) -> Optional[tensorplay.Tensor]:
    T = right_context.size(0) + utterance.size(0) + summary.size(0)
    B = right_context.size(1)
    if B == 1:
        padding_mask = None
    else:
        right_context_blocks_length = T - tensorplay.max(lengths).int() - summary.size(0)
...
def _get_activation_module(activation: str) -> tensorplay.nn.Module:
    if activation == "relu":
        return tensorplay.nn.ReLU()
    elif activation == "gelu":
        return tensorplay.nn.GELU()
    elif activation == "silu":
        return tensorplay.nn.SiLU()
    else:
...
def _gen_attention_mask_block(
    col_widths: List[int], col_mask: List[bool], num_rows: int, device: tensorplay.device
) -> tensorplay.Tensor:
    if len(col_widths) != len(col_mask):
        raise ValueError("Length of col_widths must match that of col_mask")

    mask_block = [
        tensorplay.ones(num_rows, col_width, device=device)
        if is_ones_col
        else tensorplay.zeros(num_rows, col_width, device=device)
        for col_width, is_ones_col in zip(col_widths, col_mask)
    ]
    return tensorplay.cat(mask_block, dim=1)


class _EmformerAttention(tensorplay.nn.Module):
    ...
    def __init__(
        ...
        self.emb_to_key_value = tensorplay.nn.Linear(input_dim, 2 * input_dim, bias=True)
        self.emb_to_query = tensorplay.nn.Linear(input_dim, input_dim, bias=True)
        self.out_proj = tensorplay.nn.Linear(input_dim, input_dim, bias=True)

        if weight_init_gain:
            tensorplay.nn.init.xavier_uniform_(self.emb_to_key_value.weight, gain=weight_init_gain)
            tensorplay.nn.init.xavier_uniform_(self.emb_to_query.weight, gain=weight_init_gain)

    def _gen_key_value(self, input: tensorplay.Tensor, mems: tensorplay.Tensor) -> Tuple[tensorplay.Tensor, tensorplay.Tensor]:
        T, _, _ = input.shape
        summary_length = mems.size(0) + 1
        right_ctx_utterance_block = input[: T - summary_length]
        mems_right_ctx_utterance_block = tensorplay.cat([mems, right_ctx_utterance_block])
        key, value = self.emb_to_key_value(mems_right_ctx_utterance_block).chunk(chunks=2, dim=2)
        return key, value

    def _gen_attention_probs(
        self,
        attention_weights: tensorplay.Tensor,
        attention_mask: tensorplay.Tensor,
        padding_mask: Optional[tensorplay.Tensor],
    ) -> tensorplay.Tensor:
        ...
        if padding_mask is not None:
            ...
            attention_weights_float = attention_weights_float.masked_fill(
                padding_mask.unsqueeze(1).unsqueeze(2).to(tensorplay.bool), self.negative_inf
            )
            attention_weights_float = attention_weights_float.view(B * self.num_heads, T, -1)
        attention_probs = tensorplay.nn.functional.softmax(attention_weights_float, dim=-1).type_as(attention_weights)
        return tensorplay.nn.functional.dropout(attention_probs, p=float(self.dropout), training=self.training)
    ...
    def _forward_impl(
        ...
        attention_mask: tensorplay.Tensor,
        left_context_key: Optional[tensorplay.Tensor] = None,
        left_context_val: Optional[tensorplay.Tensor] = None,
        ...
        query = self.emb_to_query(tensorplay.cat([right_context, utterance, summary]))

        # Compute key and value with [mems, right context, utterance].
        key, value = self.emb_to_key_value(tensorplay.cat([mems, right_context, utterance])).chunk(chunks=2, dim=2)
        ...
        attention_weights = tensorplay.bmm(reshaped_query * self.scaling, reshaped_key.transpose(1, 2))
        ...
        # Compute attention.
        attention = tensorplay.bmm(attention_probs, reshaped_value)
        if attention.shape != (
            B * self.num_heads,
            T,
        ...
        if self.tanh_on_mem:
            output_mems = tensorplay.tanh(output_mems)
        else:
            output_mems = tensorplay.clamp(output_mems, min=-10, max=10)
    ...
    def forward(
        ...
        Args:
            utterance (tensorplay.Tensor): utterance frames, with shape `(T, B, D)`.
            lengths (tensorplay.Tensor): with shape `(B,)` and i-th element representing
                number of valid frames for i-th batch element in ``utterance``.
            right_context (tensorplay.Tensor): right context frames, with shape `(R, B, D)`.
            summary (tensorplay.Tensor): summary elements, with shape `(S, B, D)`.
            mems (tensorplay.Tensor): memory elements, with shape `(M, B, D)`.
            attention_mask (tensorplay.Tensor): attention mask for underlying attention module.
    ...
    def infer(
        ...
        summary: tensorplay.Tensor,
        mems: tensorplay.Tensor,
        left_context_key: tensorplay.Tensor,
        left_context_val: tensorplay.Tensor,
    ) -> Tuple[tensorplay.Tensor, tensorplay.Tensor, tensorplay.Tensor, tensorplay.Tensor]:
        ...
        Args:
            ...
            summary (tensorplay.Tensor): summary elements, with shape `(S, B, D)`.
            mems (tensorplay.Tensor): memory elements, with shape `(M, B, D)`.
            left_context_key (tensorplay.Tensor): left context attention key computed from preceding invocation.
            left_context_val (tensorplay.Tensor): left context attention value computed from preceding invocation.
        ...
        attention_mask = tensorplay.zeros(query_dim, key_dim).to(dtype=tensorplay.bool, device=utterance.device)
...
class _EmformerLayer(tensorplay.nn.Module):
    ...
    def __init__(
        ...
        self.dropout = tensorplay.nn.Dropout(dropout)
        self.memory_op = tensorplay.nn.AvgPool1d(kernel_size=segment_length, stride=segment_length, ceil_mode=True)

        activation_module = _get_activation_module(activation)
        self.pos_ff = tensorplay.nn.Sequential(
            tensorplay.nn.LayerNorm(input_dim),
            tensorplay.nn.Linear(input_dim, ffn_dim),
            activation_module,
            tensorplay.nn.Dropout(dropout),
            tensorplay.nn.Linear(ffn_dim, input_dim),
            tensorplay.nn.Dropout(dropout),
        )
        self.layer_norm_input = tensorplay.nn.LayerNorm(input_dim)
        self.layer_norm_output = tensorplay.nn.LayerNorm(input_dim)
    ...
    def _init_state(self, batch_size: int, device: Optional[tensorplay.device]) -> List[tensorplay.Tensor]:
        empty_memory = tensorplay.zeros(self.max_memory_size, batch_size, self.input_dim, device=device)
        left_context_key = tensorplay.zeros(self.left_context_length, batch_size, self.input_dim, device=device)
        left_context_val = tensorplay.zeros(self.left_context_length, batch_size, self.input_dim, device=device)
        past_length = tensorplay.zeros(1, batch_size, dtype=tensorplay.int32, device=device)
        return [empty_memory, left_context_key, left_context_val, past_length]

    def _unpack_state(self, state: List[tensorplay.Tensor]) -> Tuple[tensorplay.Tensor, tensorplay.Tensor, tensorplay.Tensor]:
    ...
    def _pack_state(
        self,
        next_k: tensorplay.Tensor,
        next_v: tensorplay.Tensor,
        update_length: int,
        mems: tensorplay.Tensor,
        state: List[tensorplay.Tensor],
    ) -> List[tensorplay.Tensor]:
        new_k = tensorplay.cat([state[1], next_k])
        new_v = tensorplay.cat([state[2], next_v])
        state[0] = tensorplay.cat([state[0], mems])[-self.max_memory_size :]
    ...
    def _process_attention_output(
        self,
        rc_output: tensorplay.Tensor,
        utterance: tensorplay.Tensor,
        right_context: tensorplay.Tensor,
    ) -> tensorplay.Tensor:
        result = self.dropout(rc_output) + tensorplay.cat([right_context, utterance])
        result = self.pos_ff(result) + result
        result = self.layer_norm_output(result)
        return result

    def _apply_pre_attention_layer_norm(
        self, utterance: tensorplay.Tensor, right_context: tensorplay.Tensor
    ) -> Tuple[tensorplay.Tensor, tensorplay.Tensor]:
        layer_norm_input = self.layer_norm_input(tensorplay.cat([right_context, utterance]))
        return (
    ...
    def _apply_post_attention_ffn(
        self, rc_output: tensorplay.Tensor, utterance: tensorplay.Tensor, right_context: tensorplay.Tensor
    ) -> Tuple[tensorplay.Tensor, tensorplay.Tensor]:
    ...
    def _apply_attention_forward(
        ...
        right_context: tensorplay.Tensor,
        mems: tensorplay.Tensor,
        attention_mask: Optional[tensorplay.Tensor],
    ) -> Tuple[tensorplay.Tensor, tensorplay.Tensor]:
        ...
        if self.use_mem:
            ...
            summary = tensorplay.empty(0).to(dtype=utterance.dtype, device=utterance.device)
    ...
    def _apply_attention_infer(
        ...
        right_context: tensorplay.Tensor,
        mems: tensorplay.Tensor,
        state: Optional[List[tensorplay.Tensor]],
    ) -> Tuple[tensorplay.Tensor, tensorplay.Tensor, List[tensorplay.Tensor]]:
        if state is None:
    ...
    def infer(
        ...
        state: Optional[List[tensorplay.Tensor]],
        mems: tensorplay.Tensor,
    ) -> Tuple[tensorplay.Tensor, tensorplay.Tensor, List[tensorplay.Tensor], tensorplay.Tensor]:
        r"""Forward pass for inference.
        ...
        Args:
            ...
            lengths (tensorplay.Tensor): with shape `(B,)` and i-th element representing
                number of valid frames for i-th batch element in ``utterance``.
            right_context (tensorplay.Tensor): right context frames, with shape `(R, B, D)`.
            state (List[tensorplay.Tensor] or None): list of tensors representing layer internal state
                generated in preceding invocation of ``infer``.
            mems (tensorplay.Tensor): memory elements, with shape `(M, B, D)`.

        Returns:
            (Tensor, Tensor, List[tensorplay.Tensor], Tensor):
                Tensor
                    encoded utterance frames, with shape `(T, B, D)`.
                Tensor
                    updated right context frames, with shape `(R, B, D)`.
...
class _EmformerImpl(tensorplay.nn.Module):
    def __init__(
        self,
        emformer_layers: tensorplay.nn.ModuleList,
        segment_length: int,
        left_context_length: int = 0,
        ...
        self.use_mem = max_memory_size > 0
        self.memory_op = tensorplay.nn.AvgPool1d(
    ...
    def _gen_right_context(self, input: tensorplay.Tensor) -> tensorplay.Tensor:
        T = input.shape[0]
        ...
        return tensorplay.cat(right_context_blocks)
    ...
    def _gen_attention_mask(self, input: tensorplay.Tensor) -> tensorplay.Tensor:
        ...
        attention_mask = (1 - tensorplay.cat([tensorplay.cat(mask) for mask in masks_to_concat])).to(tensorplay.bool)
    ...
    def forward(self, input: tensorplay.Tensor, lengths: tensorplay.Tensor) -> Tuple[tensorplay.Tensor, tensorplay.Tensor]:
        ...
        Args:
            input (tensorplay.Tensor): utterance frames right-padded with right context frames, with
        ...
        mems = (
            ...
            if self.use_mem
            else tensorplay.empty(0).to(dtype=input.dtype, device=input.device)
        )
        output = utterance
    ...
    def infer(
        ...
        lengths: tensorplay.Tensor,
        states: Optional[List[List[tensorplay.Tensor]]] = None,
        ...
        Args:
            ...
            states (List[List[tensorplay.Tensor]] or None, optional): list of lists of tensors
        ...
        output_lengths = tensorplay.clamp(lengths - self.right_context_length, min=0)
        mems = (
        ...
        output = utterance
        output_states: List[List[tensorplay.Tensor]] = []
...
class Emformer(_EmformerImpl):
    ...
    *Emformer: Efficient Memory Transformer Based Acoustic Model for Low Latency Streaming Speech Recognition*
    ...
    See Also:
        * :func:`~tensorplay.audio.models.emformer_rnnt_model`,
          :func:`~tensorplay.audio.models.emformer_rnnt_base`: factory functions.
        * :class:`tensorplay.audio.pipelines.RNNTBundle`: ASR pipelines with pretrained model.
    ...
    Examples:
        ...
        >>> input = tensorplay.rand(128, 400, 512)  # batch, num_frames, feature_dim
        >>> lengths = tensorplay.randint(1, 200, (128,))  # batch
        >>> output, lengths = emformer(input, lengths)
        >>> input = tensorplay.rand(128, 5, 512)
        >>> lengths = tensorplay.ones(128) * 5
