...
Hypothesis = Tuple[List[int], tensorplay.Tensor, List[List[tensorplay.Tensor]], float]
...
def _get_hypo_predictor_out(hypo: Hypothesis) -> tensorplay.Tensor:
    return hypo[1]


def _get_hypo_state(hypo: Hypothesis) -> List[List[tensorplay.Tensor]]:
    return hypo[2]
...
def _batch_state(hypos: List[Hypothesis]) -> List[List[tensorplay.Tensor]]:
    states: List[List[tensorplay.Tensor]] = []
    for i in range(len(_get_hypo_state(hypos[0]))):
        batched_state_components: List[tensorplay.Tensor] = []
        for j in range(len(_get_hypo_state(hypos[0])[i])):
            batched_state_components.append(tensorplay.cat([_get_hypo_state(hypo)[i][j] for hypo in hypos]))
        states.append(batched_state_components)
    return states


def _slice_state(states: List[List[tensorplay.Tensor]], idx: int, device: tensorplay.device) -> List[List[tensorplay.Tensor]]:
    idx_tensor = tensorplay.tensor([idx], device=device)
...
def _compute_updated_scores(
    ...
    next_token_probs: tensorplay.Tensor,
    beam_width: int,
) -> Tuple[tensorplay.Tensor, tensorplay.Tensor, tensorplay.Tensor]:
    hypo_scores = tensorplay.tensor([_get_hypo_score(h) for h in hypos]).unsqueeze(1)
...
class RNNTBeamSearch(tensorplay.nn.Module):
    r"""Beam search decoder for RNN-T model.

    See Also:
        * :class:`tensorplay.audio.pipelines.RNNTBundle`: ASR pipeline with pretrained model.
    ...
    def _init_b_hypos(self, device: tensorplay.device) -> List[Hypothesis]:
        token = self.blank
        state = None

        one_tensor = tensorplay.tensor([1], device=device)
        pred_out, _, pred_state = self.model.predict(tensorplay.tensor([[token]], device=device), one_tensor, state)
        init_hypo = (
            [token],
            pred_out[0].detach(),
            pred_state,
    ...
    def _gen_next_token_probs(
        self, enc_out: tensorplay.Tensor, hypos: List[Hypothesis], device: tensorplay.device
    ) -> tensorplay.Tensor:
        one_tensor = tensorplay.tensor([1], device=device)
        predictor_out = tensorplay.stack([_get_hypo_predictor_out(h) for h in hypos], dim=0)
        joined_out, _, _ = self.model.join(
            enc_out,
            one_tensor,
            predictor_out,
            tensorplay.tensor([1] * len(hypos), device=device),
        )  # [beam_width, 1, 1, num_tokens]
        joined_out = tensorplay.nn.functional.log_softmax(joined_out / self.temperature, dim=3)
    ...
    def _gen_b_hypos(
        ...
        for i in range(len(a_hypos)):
            ...
            if _get_hypo_key(h_a) in key_to_b_hypo:
                ...
                score = float(tensorplay.tensor(_get_hypo_score(h_b)).logaddexp(append_blank_score))
            else:
        ...
        _, sorted_idx = tensorplay.tensor([_get_hypo_score(hypo) for hypo in b_hypos]).sort()
    ...
    def _gen_new_hypos(
        ...
        tgt_tokens = tensorplay.tensor([[token] for token in tokens], device=device)
        states = _batch_state(base_hypos)
        pred_out, _, pred_states = self.model.predict(
            tgt_tokens,
            tensorplay.tensor([1] * len(base_hypos), device=device),
            states,
        )
        new_hypos: List[Hypothesis] = []
    ...
    def _search(
        ...
        for t in range(n_time_steps):
            ...
            _, sorted_idx = tensorplay.tensor([self.hypo_sort_key(hyp) for hyp in b_hypos]).topk(beam_width)
            b_hypos = [b_hypos[idx] for idx in sorted_idx]

        return b_hypos

    def forward(self, input: tensorplay.Tensor, length: tensorplay.Tensor, beam_width: int) -> List[Hypothesis]:
        ...
        Args:
            input (tensorplay.Tensor): sequence of input frames, with shape (T, D) or (1, T, D).
            length (tensorplay.Tensor): number of valid frames in input
    ...
    def infer(
        ...
        Returns:
            (List[Hypothesis], List[List[tensorplay.Tensor]]):
                List[Hypothesis]
