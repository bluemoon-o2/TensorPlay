...
def _rendezvous_error(msg):
    return ValueError("Error initializing tensorplay.distributed using " + msg)
...
def _agent_store_serves(hostname, port) -> bool:
    """Whether the elastic agent hosts a store at ``hostname:port``."""
    return (
...
def _create_c10d_store(
    ...
    timeout_s = timeout.total_seconds() if isinstance(timeout, timedelta) else timeout
