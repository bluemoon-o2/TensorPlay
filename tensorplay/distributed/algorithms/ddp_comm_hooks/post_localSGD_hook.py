...
def post_localSGD_hook(state: PostLocalSGDState, bucket: dist.GradBucket):
    """
    ...
    :class:`~tensorplay.distributed.algorithms.model_averaging.averagers.PeriodicModelAverager`)
    ...
    """
    global_group_to_use = (
        state.process_group if state.process_group is not None else dist.GroupMember.WORLD
    )
