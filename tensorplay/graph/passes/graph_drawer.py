...
from tensorplay.graph._compatibility import compatibility
from tensorplay.graph.graph import _parse_stack_trace
from tensorplay.graph.node import _format_arg, _get_qualified_name
from tensorplay.graph.operator_schemas import normalize_function
...
if HAS_PYDOT:
    ...
    class GraphDrawer:
        """
        Visualize a tensorplay.graph.Graph with graphviz
        Basic usage:
            g = GraphDrawer(symbolic_traced, "resnet18")
            g.get_dot_graph().write_svg("a.svg")
        """

        def __init__(
            self,
            graph_module: tensorplay.graph.GraphModule,
            name: str,
            ...
            for node in graph_module.graph.nodes:
                ...
                if not isinstance(leaf_node, tensorplay.graph.GraphModule):
                    continue
        ...
        def get_dot_graph(self, submod_name: str | None = None) -> pydot.Dot:
            ...
            Example:
                ...
                >>>         super().__init__()
                >>>         self.linear = tensorplay.nn.Linear(4, 5)
                >>>     def forward(self, x):
                >>>         return self.linear(x).clamp(min=0.0, max=1.0)
                >>> module = MyModule()
                >>> # trace the module
                >>> symbolic_traced = tensorplay.graph.symbolic_trace(module)
                >>> # setup output file
                >>> import ubelt as ub
                >>> dpath = ub.Path.appdir("tensorplay/tests/GraphDrawer").ensuredir()
                >>> fpath = dpath / "linear.svg"
                >>> # draw the graph
                >>> g = GraphDrawer(symbolic_traced, "linear")
        ...
        def _get_node_style(self, node: tensorplay.graph.Node) -> dict[str, str]:
            template = {
            ...
            if node.op in _COLOR_MAP:
                ...
                # Use a stable color derived from the target name.
        ...
        def _get_leaf_node(
            self, module: tensorplay.nn.Module, node: tensorplay.graph.Node
        ...
        def _typename(self, target: tensorplay.graph.node.Target | tensorplay.nn.Module) -> str:
            if isinstance(target, tensorplay.nn.Module):
                ret = tensorplay.typename(target)
        ...
        def _get_node_label(
            self,
            module: tensorplay.graph.GraphModule,
            node: tensorplay.graph.Node,
        ...
        def _stringify_tensor_meta(self, tm: TensorMetadata) -> str:
            ...
            if tm.is_quantized:
                ...
                if qscheme in {
                    tensorplay.per_tensor_affine,
                    tensorplay.per_tensor_symmetric,
                }:
                    ...
                    tensorplay.per_channel_affine,
                    tensorplay.per_channel_symmetric,
                    tensorplay.per_channel_affine_float_qparams,
                }:
                    result += (
                        "|"
                        + "q_per_channel_scale"
        ...
        def _get_tensor_label(self, t: tensorplay.Tensor) -> str:
        ...
        def _to_dot(
            ...
            """
            Render a graph module and its node relationships.
            ...
            for node in graph_module.graph.nodes:
                ...
                def get_module_params_or_buffers() -> None:
                    for pname, ptensor in chain(
                        ...
                        label1 = (
                            ...
                            if isinstance(ptensor, tensorplay.nn.Parameter)
                            else "buffer" + r"\l"
                        )
                ...
                if node.op == "call_module":
                    ...
                    if not ignore_parameters_and_buffers and not isinstance(
                        leaf_module, tensorplay.graph.GraphModule
                    ):
    ...
    if not TYPE_CHECKING:
        ...
        class GraphDrawer:
            def __init__(
                ...
                raise RuntimeError(
                    "GraphDrawer requires the pydot package to be installed. Please install "
