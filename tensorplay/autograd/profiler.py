...
from tensorplay.profiler._chrome_trace_export import merge_distributed_traces
from tensorplay.profiler._memory_profiler import (
    export_memory_timeline,
    memory_summary,
)
from tensorplay.profiler._schedule import ProfilerAction, schedule
from tensorplay.profiler._utils import rank_world
from .profiler_util import EventList
from tensorplay.profiler.python_tracer import PySampler
...
class profile:
    ...
    def _stop_session(self):
        ...
        try:
            ...
            events.gpu_activities = self.gpu_activities
            events.mem_events = self.mem_events
            events.samples = self._sampler.samples if self._sampler is not None else []
        finally:
            self._recording = False
    ...
    def key_averages(
        ...
        events = self._ensure_events()
        if not events._tree_built:
            events._build_tree()
        if group_by_input_shape and not any(
            getattr(event, "input_shapes", None) is not None for event in events
        ):
        ...
        table = events.key_averages(
            ...
            group_by_overload_name=group_by_overload_name,
            include_python_functions=include_python_functions,
        )
