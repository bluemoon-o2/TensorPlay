...
def _pil_to_tensor(pic: Image.Image) -> tensorplay.Tensor:
    """HWC uint8 numpy -> CHW float32 [0,1] via the C++ fast path."""
    arr = np.array(pic)
    if arr.ndim == 2:
        arr = arr[:, :, None]
    if hasattr(tensorplay, "vision_to_tensor") and arr.dtype == np.uint8:
        return tensorplay.vision_to_tensor(np.ascontiguousarray(arr))
    t = tensorplay.tensor(np.ascontiguousarray(arr.transpose(2, 0, 1)))
...
def decode_jpeg(data: tensorplay.Tensor, mode: int = ImageReadMode.UNCHANGED.value, device="cpu"):
...
def encode_jpeg(img: tensorplay.Tensor, quality: int = 75) -> tensorplay.Tensor:
    ...
    return tensorplay.tensor(np.frombuffer(buf.getvalue(), dtype=np.uint8))
...
def read_image(path: str, mode: int = ImageReadMode.UNCHANGED.value) -> tensorplay.Tensor:
    """Reads an image from ``path`` and returns it as a tensor
    (tensorplay.vision.io.read_image semantics: CHW, float32 in [0,1])."""
...
def decode_image(data: tensorplay.Tensor, mode: int = ImageReadMode.UNCHANGED.value):
    """Decodes image bytes, auto-detecting the format (tensorplay.vision.io.decode_image)."""
    from io import BytesIO
