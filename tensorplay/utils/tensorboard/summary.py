...
def half_to_int(f: float) -> int:
    ...
    Converts a half precision floating point value, such as
    ``tensorplay.half`` or ``tensorplay.bfloat16``, into the unsigned 16-bit
    IEEE 754 half-precision bit pattern, which is what the half_val field of
    a TensorProto stores.
...
def int_to_half(i: int) -> float:
    ...
    Converts an unsigned 16-bit half-precision bit pattern (as stored in the
    half_val field of a TensorProto) back into a floating point value.

    """
    return float(np.uint16(i).view(np.float16))

def _tensor_to_half_val(t) -> list[int]:
    if isinstance(t, np.ndarray):
        return t.astype(np.float16).view(np.uint16).ravel().tolist()
    return t.detach().cpu().numpy().astype(np.float16).view(np.uint16).ravel().tolist()

def _tensor_to_complex_val(t) -> list[float]:
    if isinstance(t, np.ndarray):
        return np.ascontiguousarray(t).view(t.real.dtype).ravel().tolist()
    return tensorplay.view_as_real(t).flatten().tolist()

def _tensor_to_list(t) -> list[Any]:
    return t.flatten().tolist()

# type maps: tensor dtype -> (protobuf type, protobuf val field)
_TENSOR_TYPE_MAP = {
    tensorplay.half: ("DT_HALF", "half_val", _tensor_to_half_val),
    tensorplay.float16: ("DT_HALF", "half_val", _tensor_to_half_val),
    tensorplay.bfloat16: ("DT_BFLOAT16", "half_val", _tensor_to_half_val),
    tensorplay.float32: ("DT_FLOAT", "float_val", _tensor_to_list),
    tensorplay.float: ("DT_FLOAT", "float_val", _tensor_to_list),
    tensorplay.float64: ("DT_DOUBLE", "double_val", _tensor_to_list),
    tensorplay.double: ("DT_DOUBLE", "double_val", _tensor_to_list),
    tensorplay.int8: ("DT_INT8", "int_val", _tensor_to_list),
    tensorplay.uint8: ("DT_UINT8", "int_val", _tensor_to_list),
    tensorplay.qint8: ("DT_UINT8", "int_val", _tensor_to_list),
    tensorplay.int16: ("DT_INT16", "int_val", _tensor_to_list),
    tensorplay.short: ("DT_INT16", "int_val", _tensor_to_list),
    tensorplay.int: ("DT_INT32", "int_val", _tensor_to_list),
    tensorplay.int32: ("DT_INT32", "int_val", _tensor_to_list),
    tensorplay.qint32: ("DT_INT32", "int_val", _tensor_to_list),
    tensorplay.int64: ("DT_INT64", "int64_val", _tensor_to_list),
    tensorplay.complex32: ("DT_COMPLEX32", "scomplex_val", _tensor_to_complex_val),
    tensorplay.chalf: ("DT_COMPLEX32", "scomplex_val", _tensor_to_complex_val),
    tensorplay.complex64: ("DT_COMPLEX64", "scomplex_val", _tensor_to_complex_val),
    tensorplay.cfloat: ("DT_COMPLEX64", "scomplex_val", _tensor_to_complex_val),
    tensorplay.bool: ("DT_BOOL", "bool_val", _tensor_to_list),
    tensorplay.complex128: ("DT_COMPLEX128", "dcomplex_val", _tensor_to_complex_val),
    tensorplay.cdouble: ("DT_COMPLEX128", "dcomplex_val", _tensor_to_complex_val),
}
...
def hparams(hparam_dict=None, metric_dict=None, hparam_domain_discrete=None):
    ...
    for k, v in hparam_dict.items():
        ...
        if isinstance(v, tensorplay.Tensor):
            ...
            continue
        raise ValueError(
            "value should be one of int, float, str, bool, or tensorplay.Tensor"
        )
...
def tensor_proto(tag, tensor):
    ...
    if not isinstance(tensor, np.ndarray):
        dtype_key = tensor.dtype
        tensor = tensor.detach().cpu().numpy()
    else:
        dtype_key = tensor.dtype
    if tensor.size * tensor.itemsize >= (1 << 31):
        raise ValueError(
            "tensor is bigger than protocol buffer's hard limit of 2GB in size"
        )

    if dtype_key in _TENSOR_TYPE_MAP:
        dtype, field_name, conversion_fn = _TENSOR_TYPE_MAP[dtype_key]
        ...
        raise ValueError(f"{tag} has unsupported tensor dtype {dtype_key}")
...
def _get_tensor_summary(
    ...
    tensor = tensorplay.as_tensor(tensor)
