"""Image / video helpers shared by the summary builders."""

import numpy as np
import numpy.typing as npt


def figure_to_image(figures, close=True) -> np.ndarray:
    """Render matplotlib figures into a CHW uint8 image array.
    ...
    return render_to_rgb(figures)


def _prepare_video(V):
    """Spread a 5D video tensor into 4D frame grids.

    Input layout is [batch, time, channel, height, width]; each frame packs
    the batch into a near-square grid, giving [time, grid_height, grid_width,
    channel].
    """
    b, t, c, h, w = V.shape
...
def convert_to_HWC(tensor, input_format):  # tensor: numpy array
    ...
    if len(input_format) == 2:
        ...
        return tensor
    raise AssertionError("input_format must have length 2, 3 or 4")
