...
At the heart of TensorPlay data loading utility is the {class}`tensorplay.utils.data.DataLoader`
...
{class}`~tensorplay.utils.data.DataLoader`, which has signature:
```python
...
The most important argument of {class}`~tensorplay.utils.data.DataLoader`
constructor is {attr}`dataset`, which indicates a dataset object to load data
from. TensorPlay supports two different types of datasets:
...
See {class}`~tensorplay.utils.data.Dataset` for more details.
(iterable-style-datasets)=

### Iterable-style datasets

An iterable-style dataset is an instance of a subclass of {class}`~tensorplay.utils.data.IterableDataset`
...
in real time.
See {class}`~tensorplay.utils.data.IterableDataset` for more details.
:::{note}
When using a {class}`~tensorplay.utils.data.IterableDataset` with
...
{class}`~tensorplay.utils.data.IterableDataset` documentations for how to
achieve this.
:::
(data-loading-order-and-sampler)=

## Data Loading Order and {class}`~tensorplay.utils.data.Sampler`
...
{ref}`map-style datasets <map-style-datasets>`. {class}`tensorplay.utils.data.Sampler`
...
{class}`~tensorplay.utils.data.Sampler` could randomly permute a list of indices
and yield each one at a time, or yield a small number of them for mini-batch
SGD.
A sequential or shuffled sampler will be automatically constructed based on the {attr}`shuffle` argument to a {class}`~tensorplay.utils.data.DataLoader`.
Alternatively, users may use the {attr}`sampler` argument to specify a
custom {class}`~tensorplay.utils.data.Sampler` object that at each time yields
the next index/key to fetch.
A custom {class}`~tensorplay.utils.data.Sampler` that yields a list of batch
...
{class}`~tensorplay.utils.data.DataLoader` supports automatically collating
...
converts NumPy arrays into TensorPlay Tensors, and keeps everything else untouched.
In this case, loading from a map-style dataset is roughly equivalent with:
```python
...
({func}`~tensorplay.utils.data.default_collate`).
...
If you run into a situation where the outputs of {class}`~tensorplay.utils.data.DataLoader`
...
A {class}`~tensorplay.utils.data.DataLoader` uses single-process data loading by
default.
...
computation code with data loading, TensorPlay provides an easy switch to perform
...
{class}`~tensorplay.utils.data.DataLoader` is initialized. Therefore, data loading
...
[issue #13246](https://github.com/tensorplay/tensorplay/issues/13246#issuecomment-905703662)
for more details on why this occurs and example code for how to
workaround these problems.
:::
In this mode, each time an iterator of a {class}`~tensorplay.utils.data.DataLoader`
...
{func}`tensorplay.utils.data.get_worker_info()` returns various useful information
...
duplicated data. Using {func}`tensorplay.utils.data.get_worker_info()` and/or
{attr}`worker_init_fn`, users may configure each replica independently. (See
{class}`~tensorplay.utils.data.IterableDataset` documentations for how to achieve
...
- Wrap most of you main script's code within `if __name__ == '__main__':` block,
  ...
  process is launched. You can place your dataset and {class}`~tensorplay.utils.data.DataLoader`
...
By default, each worker will have its TensorPlay seed set to `base_seed + worker_id`,
...
In {attr}`worker_init_fn`, you may access the TensorPlay seed set for each worker
with either {func}`tensorplay.utils.data.get_worker_info().seed <tensorplay.utils.data.get_worker_info>`
or {func}`tensorplay.initial_seed()`, and use it to seed other libraries before data
loading.
(memory-pinning)=
...
class SimpleCustomBatch:
    def __init__(self, data):
        ...
        self.inp = tensorplay.stack(transposed_data[0], 0)
        self.tgt = tensorplay.stack(transposed_data[1], 0)
...
inps = tensorplay.arange(10 * 5, dtype=tensorplay.float32).view(10, 5)
tgts = tensorplay.arange(10 * 5, dtype=tensorplay.float32).view(10, 5)
...
```
% These modules are documented as part of tensorplay/data listing them here for
% now until we have a clearer fix
```{eval-rst}
.. py:module:: tensorplay.utils.data.datapipes
```
```{eval-rst}
.. py:module:: tensorplay.utils.data.datapipes.dataframe
```
```{eval-rst}
.. py:module:: tensorplay.utils.data.datapipes.iter
```
```{eval-rst}
.. py:module:: tensorplay.utils.data.datapipes.map
```
```{eval-rst}
.. py:module:: tensorplay.utils.data.datapipes.utils
```
