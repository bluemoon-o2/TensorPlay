...
{meth}`~tensorplay.autograd.Function.apply` ed to compute the result of
evaluating the graph.  When computing the forward pass, autograd
simultaneously performs the requested computations and builds up a graph
representing the function that computes the gradient (the ``.grad_fn``
attribute of each {class}`tensorplay.Tensor` is an entry point into this graph).
...
When defining a custom Python {class}`~tensorplay.autograd.Function`, you can use
...
```python
x = tensorplay.randn(5, requires_grad=True)
y = x.pow(2)
...
Under the hood, to prevent reference cycles, TensorPlay has *packed* the tensor
...
When performing division by zero in TensorPlay (e.g., ``x / 0``), the forward pass will produce ``inf`` values following IEEE-754 floating point arithmetic. While these ``inf`` values can be masked out before computing the final loss (e.g., via indexing or masking), the autograd system still tracks and differentiates through the full computation graph, including the division by zero operation.

During backpropagation, this can lead to problematic gradient expressions. For example:

```python
x = tensorplay.tensor([1., 1.], requires_grad=True)
div = tensorplay.tensor([0., 1.])
...
```python
from tensorplay.masked import as_masked_tensor
...
be selected from Python that can affect how computations in TensorPlay are
...
enabling inference mode will allow TensorPlay to speed up your model even more.
...
[Inference Mode](https://www.tensorplay.cn/docs/docs/2.9/notes/autograd.html#inference-mode).

For implementation details of inference mode see
[RFC-0011-InferenceMode](https://github.com/tensorplay/rfcs/pull/17).
...
model relies on modules such as {class}`tensorplay.nn.Dropout` and
{class}`tensorplay.nn.BatchNorm2d` that may behave
...
def train_fn():
    x = tensorplay.ones(5, 5, requires_grad=True)
    # forward
    y = (x + 3) * (x + 4) * 0.5
    # backward
    y.sum().backward()
...
The functional API {func}`tensorplay.autograd.grad` may be used to calculate the
...
- When you use TensorPlay to differentiate any function $f(z)$ with complex domain and/or codomain,
...
to define complex derivatives in TensorPlay, read on.
...
Because the conjugate Wirtinger derivative gives us exactly the correct step for a real valued loss function, TensorPlay gives you this derivative
when you differentiate a function with a real valued loss.

### How does TensorPlay compute the conjugate Wirtinger derivative?
...
Using {eq}`eq4`, and `grad\_output = 1.0` (which is the default grad output value used when {func}`backward` is called on a scalar output in TensorPlay), we get:
...
def pack_hook(tensor):
    ...
    tensorplay.save(tensor, temp_file.name)
    return temp_file

def unpack_hook(temp_file):
    return tensorplay.load(temp_file.name)
```
...
```{warning}
 ...
 as they may lead to unexpected side-effects. TensorPlay will throw an error if the
...
{meth}`~tensorplay.autograd.SavedTensor.register_hooks` method on a
...
```{warning}
 ...
 its {meth}`~tensorplay.autograd.SavedTensor.register_hooks` is forbidden.
 TensorPlay will throw an error most of the time but it may fail
...
{class}`~tensorplay.autograd.graph.saved_tensors_hooks` to register a pair of
...
```{note}
 ...
 {meth}`~tensorplay.autograd.SavedTensor.register_hooks` above, which receives an
 ...
 alive, and because TensorPlay does not expose the autograd graph to Python's
 cyclic garbage collector (tensorplay/tensorplay#7343) it cannot be reclaimed even by
 ...
 is lossless: as noted above, TensorPlay stashes the autograd metadata separately
...
def unpack_hook(tensor_or_sctf):
    if isinstance(tensor_or_sctf, tensorplay.Tensor):
        return tensor_or_sctf
    return tensorplay.load(tensor_or_sctf.name)

class Model(nn.Module):
    def forward(self, x):
        with tensorplay.autograd.graph.saved_tensors_hooks(pack_hook, unpack_hook):
          # ... compute output
          output = x
        return output
...
```python
with tensorplay.autograd.graph.saved_tensors_hooks(lambda x: x.detach(), lambda x: x):
    x = tensorplay.randn(5, requires_grad=True)
    y = x * x
```
...
With the hooks, TensorPlay will pack and unpack `x` into two new tensor objects
...
Hooks registered to a Tensor via {meth}`tensorplay.Tensor.register_hook`
...
as part of the ``inputs`` argument to {func}`tensorplay.autograd.grad`,
the Tensor's grad_fn may not be executed, but the hook register to that Tensor will always be executed.)

Hooks registered to a Tensor via {meth}`tensorplay.Tensor.register_post_accumulate_grad_hook`
are executed after the gradients have been accumulated for that Tensor, meaning the
Tensor's grad field has been set. Whereas hooks registered via {meth}`tensorplay.Tensor.register_hook`
are run as gradients are being computed, hooks registered via {meth}`tensorplay.Tensor.register_post_accumulate_grad_hook`
...
{func}`tensorplay.autograd.grad` or {func}`tensorplay.autograd.backward`.
Specifically, you should be aware of these differences when you register a hook on a
Node corresponding to a Tensor that you are passing to {func}`tensorplay.autograd.grad` or
{func}`tensorplay.autograd.backward` as part of the ``inputs`` argument.

If you are using {func}`tensorplay.autograd.backward`, all of the above mentioned hooks will be executed,
...
On the other hand, if you are using {func}`tensorplay.autograd.grad`, the backward hooks registered
...
```python
t = tensorplay.tensor(1., requires_grad=True).sin()
t.cos_()
t.register_hook(fn)
t.backward()
```
