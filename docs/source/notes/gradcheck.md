...
This note presents an overview of how the {meth}`~tensorplay.autograd.gradcheck` and {meth}`~tensorplay.autograd.gradgradcheck` functions work.
...
To simplify internal computations, TensorPlay uses {math}`2 * CW` as the value it backwards and returns when the user asks for gradients.
...
```python
# Code from https://github.com/tensorplay/tensorplay/blob/58eb23378f2a376565a66ac32c93a316c45b6131/tensorplay/autograd/gradcheck.py#L99-L105
...
TensorPlay also provide a utility to verify second order gradients. The goal here is to make sure that the backward implementation is also properly differentiable and computes the right thing.
