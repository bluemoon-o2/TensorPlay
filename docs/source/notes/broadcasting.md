...
Many TensorPlay operations support NumPy's broadcasting semantics.
See [Numpy Documentation](https://numpy.org/doc/stable/user/basics.broadcasting.html) for details.

In short, if a TensorPlay operation supports broadcast, then its Tensor arguments can be
...
```python
>>> x=tensorplay.empty(5,7,3)
>>> y=tensorplay.empty(5,7,3)
# same shapes are always broadcastable (i.e. the above rules always hold)

>>> x=tensorplay.empty((0,))
>>> y=tensorplay.empty(2,2)
# x and y are not broadcastable, because the 0-sized dimension of x
# does not match the 2-sized dimension of y.

# can line up trailing dimensions
>>> x=tensorplay.empty(5,3,4,1)
>>> y=tensorplay.empty(  3,1,1)
...
>>> x=tensorplay.empty(5,1,4,1)
>>> y=tensorplay.empty(  3,1,1)
>>> (x+y).size()
tensorplay.Size([5, 3, 4, 1])

# but not necessary:
>>> x=tensorplay.empty(1)
>>> y=tensorplay.empty(3,1,7)
>>> (x+y).size()
tensorplay.Size([3, 1, 7])
...
```python
>>> tensorplay.add(tensorplay.ones(4,1), tensorplay.randn(4))
```

would previously produce a Tensor with size: tensorplay.Size([4,1]), but now produces a Tensor with size: tensorplay.Size([4,4]).
In order to help identify cases in your code where backwards incompatibilities introduced by broadcasting may exist,
you may set `tensorplay.utils.backcompat.broadcast_warning.enabled` to `True`, which will generate a python warning
in such cases.

For Example:

```python
>>> tensorplay.utils.backcompat.broadcast_warning.enabled=True
>>> tensorplay.add(tensorplay.ones(4,1), tensorplay.ones(4))
