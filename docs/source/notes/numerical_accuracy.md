...
Because of this, TensorPlay is not guaranteed
...
TensorPlay releases, individual commits, or different platforms. In particular, CPU and GPU
...
Many operations in TensorPlay support batched computation, where the same operation is performed
for the elements of the batches of inputs. An example of this is {func}`tensorplay.mm` and
{func}`tensorplay.bmm`. It is possible to implement batched computation as a loop over batch
...
batch. The mathematical libraries that we are calling, and TensorPlay internal implementations
...
```python
import tensorplay
a = tensorplay.tensor([1e20, 1e20])  # fp32 type by default
a.norm()  # produces tensor(inf)
a.double().norm()  # produces tensor(1.4142e+20, dtype=tensorplay.float64), representable in fp32
```
...
consider normalizing or clamping extremal inputs and using {func}`tensorplay.isfinite` to detect
them.

(Linear Algebra Stability)=

## Linear algebra (`tensorplay.linalg`)

(non-finite-values)=

### Non-finite values

The external libraries (backends) that `tensorplay.linalg` uses provide no guarantees on their
behaviour when the inputs have non-finite values like `inf` or `NaN`. As such, neither does
TensorPlay. The operations may return a tensor with non-finite values, or raise an exception,
or even segfault.

Consider using {func}`tensorplay.isfinite` before calling these functions to detect this
situation.
...
Functions within `tensorplay.linalg` have more {ref}`extremal values <extremal-values>` than
other TensorPlay functions.
...
{func}`tensorplay.linalg.svdvals` or their condition number via {func}`tensorplay.linalg.cond` may
...
`tensorplay.backends.cuda.matmul.fp32_precision = "tf32"` (`tensorplay.backends.cuda.matmul.allow_tf32 = True`
...
`tensorplay.backends.cudnn.conv.fp32_precision = "ieee"` (`tensorplay.backends.cudnn.allow_tf32 = False`
...
`tensorplay.backends.cuda.matmul.allow_fp16_reduced_precision_reduction = False`.
This sets `allow_splitk` to `True`. To disable both reduced-precision reductions
and split-k reductions, use
`tensorplay.backends.cuda.matmul.allow_fp16_reduced_precision_reduction = (False, False)`.
...
`tensorplay.backends.cuda.matmul.allow_bf16_reduced_precision_reduction = False`.
This sets `allow_splitk` to `True`. To disable both reduced-precision reductions
and split-k reductions, use
`tensorplay.backends.cuda.matmul.allow_bf16_reduced_precision_reduction = (False, False)`.
...
with the following setting: `tensorplay.backends.cuda.allow_fp16_bf16_reduction_math_sdp(True)`.
...
rocBLAS (GEMM) and MIOpen (convolution) kernels; all other TensorPlay operations will not
...
during gradient calculation. TensorPlay by default will use the rocBLAS and MIOpen alternate
...
- `tensorplay.bmm`
- `tensorplay.mm`
- `tensorplay.nn.GRUCell`
- `tensorplay.nn.LSTMCell`
- `tensorplay.nn.Linear`
- `tensorplay.sparse.addmm`
- the following `tensorplay._C._ConvBackend` implementations:
  - `slowNd`
  - `slowNd_transposed`
  - `slowNd_dilated`
...
- `tensorplay.nn.Conv[Transpose]Nd`
- the following `tensorplay._C._ConvBackend` implementations:
  - `ConvBackend::Miopen`
...
```python
tensorplay.backends.cuda.matmul.fp32_precision = "ieee"
```

To enable TF32 on supported hardware:

```python
tensorplay.backends.cuda.matmul.fp32_precision = "tf32"
```
