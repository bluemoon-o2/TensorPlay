...
To use {mod}`tensorplay.optim` you have to construct an optimizer object that will hold
the current state and will update the parameters based on the computed gradients.

### Constructing it

To construct an {class}`Optimizer` you have to give it an iterable containing the
parameters (all should be {class}`~tensorplay.nn.Parameter` s) or named parameters
(tuples of (str, {class}`~tensorplay.nn.Parameter`)) to optimize. Then,
...
of passing an iterable of {class}`~tensorplay.autograd.Variable` s, pass in an iterable of
...
Remember that {func}`~tensorplay.nn.Module.parameters` returns an iterable that
...
{func}`~tensorplay.autograd.Variable.backward`.
Example:
```python
...
{class}`tensorplay.optim.lr_scheduler.LRScheduler` provides several methods to adjust the learning
rate based on the number of epochs. {class}`tensorplay.optim.lr_scheduler.ReduceLROnPlateau`
...
```
```{warning}
Prior to TensorPlay 1.1.0, the learning rate scheduler was expected to be called before
...
If you are unable to reproduce results after upgrading to TensorPlay 1.1.0, please check
...
optimizer = optim.SGD(model.named_parameters(), lr=0.01, momentum=0.9)
# training..
tensorplay.save(optimizer.state_dict(), PATH)
```
...
def adapt_state_dict_ids(optimizer, state_dict):
    ...
    clone_deepcopy = lambda d: {k: (v.clone() if isinstance(v, tensorplay.Tensor) else deepcopy(v)) for k, v in d.items()}
...
optimizer2.load_state_dict(tensorplay.load(PATH)) # The previous optimizer saved state_dict
```
...
class Model_bypass(nn.Module):
    def __init__(self):
        ...
        tensorplay.nn.init.eye_(self.bypass.weight)
...
{class}`tensorplay.optim.swa_utils.AveragedModel` implements Stochastic Weight Averaging (SWA) and Exponential Moving Average (EMA),
{class}`tensorplay.optim.swa_utils.SWALR` implements the SWA learning rate scheduler and
{func}`tensorplay.optim.swa_utils.update_bn` is a utility function used to update SWA/EMA batch
...
```
Decay is a parameter between 0 and 1 that controls how fast the averaged parameters are decayed. If not provided to {func}`tensorplay.optim.swa_utils.get_ema_multi_avg_fn`, the default is 0.999. Decay value should be close to 1.0, as smaller values can cause optimization convergence issues.
{func}`tensorplay.optim.swa_utils.get_ema_multi_avg_fn` returns a function that applies the following EMA equation to the weights:
```{math}
...
Here the model `model` can be an arbitrary {class}`tensorplay.nn.Module` object. `averaged_model`
...
By default, {class}`tensorplay.optim.swa_utils.AveragedModel` computes a running equal average of
...
- `multi_avg_fn` allows defining more efficient operations acting on a tuple of parameter lists, (averaged parameter list, model parameter list), at the same time, for example using the `tensorplay._foreach*` functions. This function must update the averaged parameters in-place.
...
>>> ema_model = tensorplay.optim.swa_utils.AveragedModel(model, avg_fn=ema_avg)
```
...
```python
>>> swa_scheduler = tensorplay.optim.swa_utils.SWALR(optimizer, \
...
```python
>>> tensorplay.optim.swa_utils.update_bn(loader, swa_model)
```
...
>>> swa_model = tensorplay.optim.swa_utils.AveragedModel(model)
>>> scheduler = tensorplay.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=300)
>>> swa_start = 160
...
```python
>>> loader, optimizer, model, loss_fn = ...
>>> ema_model = tensorplay.optim.swa_utils.AveragedModel(model, \
>>>             multi_avg_fn=tensorplay.optim.swa_utils.get_ema_multi_avg_fn(0.999))
>>>
>>> for epoch in range(300):
>>>       for input, target in loader:
>>>           optimizer.zero_grad()
...
>>> tensorplay.optim.swa_utils.update_bn(loader, ema_model)
