...
static void tp_body(void* ctxp, long long b, long long e) {
    ...
    for (; i + 4 * W <= e; i += 4 * W) {
        ...
        V t10 = tensorplay::vec::minimum(x10, V(0.0f));
        V t20 = (t00 + t10);
        V t01 = tensorplay::vec::maximum(x01, V(0.0f));
        V t11 = tensorplay::vec::minimum(x11, V(0.0f));
        V t21 = (t01 + t11);
        V t02 = tensorplay::vec::maximum(x02, V(0.0f));
        V t12 = tensorplay::vec::minimum(x12, V(0.0f));
        V t22 = (t02 + t12);
        V t03 = tensorplay::vec::maximum(x03, V(0.0f));
        V t13 = tensorplay::vec::minimum(x13, V(0.0f));
        V t23 = (t03 + t13);
        t20.store(out + i, W);
    ...
    for (; i + W <= e; i += W) {
        ...
        V t1 = tensorplay::vec::minimum(x1, V(0.0f));
        V t2 = (t0 + t1);
        t2.store(out + i, W);
    }
    if (i < e) {
...
extern "C" void tp_native_08d6983b6ad0fe6d(long n, const float* __restrict__ in0, const float* __restrict__ in1, float* __restrict__ out) {
