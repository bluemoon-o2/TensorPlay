...
Tensor rand_kernel(const std::vector<int64_t>& size, DType dtype, Device device, bool requires_grad) {
...
Tensor zeros_kernel(const std::vector<int64_t>& size, DType dtype, Device device, bool requires_grad) {
    Tensor t(size, dtype, device);
    t.set_requires_grad(requires_grad);
...
Tensor ones_kernel(const std::vector<int64_t>& size, DType dtype, Device device, bool requires_grad) {
    Tensor t(size, dtype, device);
    t.set_requires_grad(requires_grad);
    fill_kernel(t, 1);
    return t;
...
Tensor arange_start_step_kernel(Scalar start, Scalar end, Scalar step, DType dtype, Device device, bool requires_grad) {
...
Tensor arange_kernel(Scalar end, DType dtype, Device device, bool requires_grad) {
    return arange_start_step_kernel(Scalar(0), end, Scalar(1), dtype, device, requires_grad);
}

Tensor empty_kernel(const std::vector<int64_t>& size, DType dtype, Device device, bool requires_grad) {
    Tensor t(size, dtype, device);
    t.set_requires_grad(requires_grad);
    return t;
}
...
Tensor linspace_kernel(Scalar start, Scalar end, int64_t steps, DType dtype, Device device, bool requires_grad) {
...
Tensor logspace_kernel(Scalar start, Scalar end, int64_t steps, double base, DType dtype, Device device, bool requires_grad) {
    if (steps < 0) TP_THROW(RuntimeError, "number of steps must be non-negative");
    Tensor t({steps}, dtype, device);
    t.set_requires_grad(requires_grad);
    if (steps == 0) return t;
...
Tensor randn_kernel(const std::vector<int64_t>& size, DType dtype, Device device, bool requires_grad) {
    Tensor t(size, dtype, device);
    t.set_requires_grad(requires_grad);
    if (dtype == DType::Float32) {
        float* data = t.data_ptr<float>();
...
Tensor randint_kernel(int64_t low, int64_t high, const std::vector<int64_t>& size, DType dtype, Device device, bool requires_grad) {
    Tensor t(size, dtype, device);
    t.set_requires_grad(requires_grad);
    if (dtype == DType::Int64) {
        ...
        for (int64_t i = 0; i < n; ++i) {
         ...
         TP_THROW(NotImplementedError, "randint() only supports Int64/Int32");
    }
    return t;
}

Tensor randperm_kernel(int64_t n, DType dtype, Device device, bool requires_grad) {
...
Tensor rand_like_kernel(const Tensor& self, DType dtype, Device device, bool requires_grad) {
    if (dtype == DType::Undefined) dtype = self.dtype();
    return rand_kernel(static_cast<std::vector<int64_t>>(self.shape()), dtype, device, requires_grad);
}

Tensor randint_like_kernel(const Tensor& self, int64_t low, int64_t high, DType dtype, Device device, bool requires_grad) {
    if (dtype == DType::Undefined) dtype = self.dtype();
    return randint_kernel(low, high, static_cast<std::vector<int64_t>>(self.shape()), dtype, device, requires_grad);
}

Tensor randn_like_kernel(const Tensor& self, DType dtype, Device device, bool requires_grad) {
    if (dtype == DType::Undefined) dtype = self.dtype();
    return randn_kernel(static_cast<std::vector<int64_t>>(self.shape()), dtype, device, requires_grad);
}
...
TENSORPLAY_REGISTER_KERNEL(arange, CPU, arange_start_step_kernel)
TENSORPLAY_REGISTER_KERNEL_STR("arange.end", CPU, arange_kernel)
TENSORPLAY_REGISTER_KERNEL(empty, CPU, empty_kernel)
TENSORPLAY_REGISTER_KERNEL(eye, CPU, eye_kernel)
TENSORPLAY_REGISTER_KERNEL(linspace, CPU, linspace_kernel)
TENSORPLAY_REGISTER_KERNEL(logspace, CPU, logspace_kernel)
TENSORPLAY_REGISTER_KERNEL_STR("fill_.Scalar", CPU, fill_kernel)

TENSORPLAY_REGISTER_KERNEL(randn, CPU, randn_kernel)
TENSORPLAY_REGISTER_KERNEL(randint, CPU, randint_kernel)
TENSORPLAY_REGISTER_KERNEL(randperm, CPU, randperm_kernel)
TENSORPLAY_REGISTER_KERNEL(rand_like, CPU, rand_like_kernel)
TENSORPLAY_REGISTER_KERNEL(randint_like, CPU, randint_like_kernel)
TENSORPLAY_REGISTER_KERNEL(randn_like, CPU, randn_like_kernel)
