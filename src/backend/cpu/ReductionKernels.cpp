...
Tensor sum_kernel(const Tensor& self, std::optional<DType> dtype) {
    DType out_dtype = dtype.value_or(self.dtype());
    // If integer, promote to Int64 to avoid overflow? PyTorch does this for sum.
    if (!dtype.has_value()) {
    ...
    case DType::name: { \
        ...
        if constexpr (std::is_same_v<ctype, bool>) { \
            for(int64_t i=0; i<n; ++i) sum_val = sum_val || data[i]; \
        } else { \
            for(int64_t i=0; i<n; ++i) sum_val += data[i]; \
        } \
...
Tensor sum_dim_kernel(const Tensor& self, std::vector<int64_t> dims, bool keepdim, std::optional<DType> dtype) {
    DType out_dtype = dtype.value_or(self.dtype());
    if (!dtype.has_value()) {
         if (isIntegralType(self.dtype())) {
             out_dtype = DType::Int64;
    ...
    case DType::name: { \
        ...
        auto recurse = [&](auto&& self_recurse, int64_t dim, int64_t inp_off, int64_t out_off) -> void { \
            if (dim == (int64_t)inp_shape.size()) { \
                if constexpr (std::is_same_v<ctype, bool>) { \
                    out_data[out_off] = out_data[out_off] || inp_data[inp_off]; \
                } else { \
...
Tensor mean_kernel(const Tensor& self, std::optional<DType> dtype) {
    DType out_dtype = dtype.value_or(isFloatingType(self.dtype()) ? self.dtype() : DType::Float32);
    
    Tensor s = sum_kernel(self, out_dtype);
    return s / Scalar((float)self.numel());
}

Tensor mean_dim_kernel(const Tensor& self, std::vector<int64_t> dims, bool keepdim, std::optional<DType> dtype) {
    DType out_dtype = dtype.value_or(isFloatingType(self.dtype()) ? self.dtype() : DType::Float32);
...
// Product
Tensor prod_kernel(const Tensor& self, std::optional<DType> dtype) {
    DType out_dtype = dtype.value_or(self.dtype());
    if (!dtype.has_value()) {
         if (isIntegralType(self.dtype())) {
             out_dtype = DType::Int64;
    ...
    case DType::name: { \
        ...
        ctype* data = self_contig.data_ptr<ctype>(); \
        int64_t n = self_contig.numel(); \
        for(int64_t i=0; i<n; ++i) prod_val *= data[i]; \
...
Tensor prod_dim_kernel(const Tensor& self, std::vector<int64_t> dims, bool keepdim, std::optional<DType> dtype) {
    DType out_dtype = dtype.value_or(self.dtype());
    if (!dtype.has_value()) {
         if (isIntegralType(self.dtype())) {
             out_dtype = DType::Int64;
