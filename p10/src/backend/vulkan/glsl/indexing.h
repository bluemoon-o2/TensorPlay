 ...
 */
ivec4 idx_to_coord(const uint idx, const uvec4 strides, const uvec4 sizes) {
...
uint coord_to_idx(const uvec4 coord, const uvec4 strides) {
  return uint(dot(coord * strides, uvec4(1)));
}
...
// Return the x, y, z and index value the channel-packed 3D tensor from the
// {n, c, h, w}-index.
