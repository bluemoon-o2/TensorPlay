...
// Int8 variant of the strided-view materialization gather: signed-byte
// textures in and out, one byte per channel lane.
...
/*
 * Generic strided gather over Int8 storage: each output texel maps its lane
 ...
 * copies the byte.  Zero strides re-read one position, which materializes
