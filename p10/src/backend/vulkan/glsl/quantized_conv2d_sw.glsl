...
/*
 * TILE_SIZE = (1, 1, 1)
 * WEIGHT_STORAGE = TEXTURE_3D (the rearranged weight carries its
 * output-channel groups of four in the texel lanes, so the third image axis
 * of the packed payload stays at depth one)
...
void main() {
  ...
  // reading the input. Note that these are "canonical" indices.  The integer
  // division assumes every in-bounds input position aligns with a kernel tap:
  // when (start - ipos) is not a multiple of the dilation the sampling grid
  // shifts, so dilation combined with padding that misaligns the grid is not
