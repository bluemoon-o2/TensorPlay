...
void main() {
  ...
  // Zero padding is added to the channels dimension when tensors are stored
  // as image textures. This will cause a divide-by-zero when performing
  // division. For division, apply an extra step of detecting which elements
  // are zero padding to avoid division by zero.
  // clang-format off
  $if IS_DIV:
