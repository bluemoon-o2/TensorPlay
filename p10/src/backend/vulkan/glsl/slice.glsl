...
/*
 * Strided slice along one axis, addressed in innermost-first axis order
 * (0: width, 1: height, 2: channel, 3: batch).  The invocation maps its
 * output coordinates back to input coordinates: the sliced axis moves by
 * start + step * position, a dropped axis pins to `start` and shifts all
 * outer axes inward by one slot.  Elements are fetched per channel lane so
 * slicing the channel axis reroutes lanes and z positions correctly.
 */
void main() {
