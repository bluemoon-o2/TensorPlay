...
/*
 * Generic strided gather: materializes the dense output of a strided view.
 * Each invocation maps its output coordinates {n, c, h, w} through the view
 * strides to a linear position in the input's logical order, decomposes that
 * position over the input's dense layout, and fetches one element per channel
 * lane.  Zero strides re-read one position, which materializes broadcast
 * expansions without a separate kernel.
 */
void main() {
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    const int linear_offset = uBlock.offset + n_out * uBlock.out_strides.x +
    ...
    const int n_in = linear_offset / uBlock.in_strides.x;
    int r = linear_offset - n_in * uBlock.in_strides.x;
    ...
    out_texel[i] = texelFetch(
        uInput,
        ivec3(w_in, h_in, n_in * uBlock.in_c_depth + c_in / 4),
        0)[c_in % 4];
  }
