...
//
// The descriptor signature of a compute shader, expressed as the ordered
// list of descriptor types bound at set 0.  Two shaders share a
// VkDescriptorSetLayout (and therefore a pool bucket) iff their signatures
// are equal.
//
...
//
// Everything needed to create (or retrieve from cache) the VkShaderModule
// and its pipeline for one compiled shader variant: the SPIR-V words, the
// descriptor signature, and the tile size the dispatch loop multiplies by.
//
  ...
  struct Hasher {
    inline size_t operator()(const ShaderLayout::Signature& signature) const {
      ...
      for (const VkDescriptorType type : signature) {
        hashed = utils::hash_combine(hashed, std::hash<VkDescriptorType>()(type));
      }
