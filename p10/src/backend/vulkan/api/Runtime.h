...
// of a Vulkan Context (i.e. logical devices).  All Vulkan tensors are
// associated with a Context to make tensor <-> device affinity explicit.
//
...
class Runtime final {
 ...
 public:
  ...
  inline bool is_initialized() const {
    return default_adapter_i_ < adapters_.size();
  }

  inline Adapter* get_adapter_p() {
    VK_CHECK_COND(
        default_adapter_i_ >= 0 && default_adapter_i_ < adapters_.size(),
        "TensorPlay Vulkan Runtime: Default device adapter is not set correctly!");
  ...
  inline Adapter* get_adapter_p(uint32_t i) {
    VK_CHECK_COND(
        ...
        "TensorPlay Vulkan Runtime: Adapter at index ",
        i,
        " is not available!");
...
// a static local variable.  Returns null when no Vulkan implementation can be
// brought up on this system.
Runtime* try_runtime();

// Same as try_runtime() but throws when the runtime is unavailable.
Runtime* runtime();
