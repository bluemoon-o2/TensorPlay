...
//
// VulkanBuffer owns one VkBuffer plus its VMA allocation.  The allocation
// request decides which memory type backs it:
//   * tensor payloads prefer device-local memory, falling back to
//     host-visible memory when no device-local heap exists;
//   * staging and params buffers always request host-visible, host-coherent
//     memory so the CPU can map them directly.
//
...
//
// Scoped mapping of a buffer's memory into the host address space.  The
// mapping is only valid for buffers allocated from host-visible memory
// (staging, params, and device-local memory that is also host-visible on
// unified-memory devices).  VMA persistently maps allocations that requested
// it; otherwise vkMapMemory / vkUnmapMemory bracket the accessor.
//
class MemoryMap final {
 ...
 public:
  ...
  inline size_t nbytes() {
    return static_cast<size_t>(data_len_);
  }

  // Flushes HOST_COHERENT-less writes so the device observes them; a no-op
  // for coherent memory.
  void invalidate();
};
...
//
// A sampled view over one image.  Samplers are cached per-properties by the
// owning adapter.
//
...
//
// A fence with its own wait/query lifecycle.  Fences are pooled; reuse only
// after the prior wait has completed.
//
...
//
// Owns a VmaAllocator instance and exposes the typed buffer-creation entry
// points used by the rest of the backend.  VMA performs the actual
// sub-allocation inside memory heaps.
//
class MemoryAllocator final {
 ...
 public:
  ...
  inline VmaAllocator handle() const {
    return allocator_;
  }
};
