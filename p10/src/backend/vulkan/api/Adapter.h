...
//
// The Runtime class holds one Adapter instance for each physical device
// visible to the VkInstance. Upon construction, this class will populate the
// physical device properties, but will not create the logical device until
// specifically requested via the init_device() function.
//
...
class Adapter final {
 ...
 public:
  ...
  inline const VkPhysicalDeviceMemoryProperties& memory_properties() const {
    return physical_device_.memory_properties;
  }

  inline uint32_t max_compute_work_group_invocations() const {
