...
//
// A thin stateful wrapper around one VkCommandBuffer.  The state machine
// enforces the legal recording order:
//   begin -> bind_pipeline -> bind_descriptors -> (barrier -> dispatch/copy)*
//   -> end -> submit
// which matches the way Context records dispatches into its shared command
// buffer.
//
...
//
// Grows a VkCommandPool in batches and hands out command buffers.  Buffers
// are reset (not destroyed) when returned to the pool at flush time.
//
