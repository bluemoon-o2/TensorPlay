// 1-D pooling wrappers delegate only to the existing native 2-D CUDA kernels
// in PoolingKernels.cu; they never fall through to Composite or Python.
...
Tensor adaptive_avg_pool1d_native_cuda(
...
std::tuple<Tensor, Tensor> adaptive_max_pool1d_native_cuda(
    const Tensor& self, const std::vector<int64_t>& output_size) {
...
std::tuple<Tensor, Tensor> max_pool1d_with_indices_native_cuda(
    const Tensor& self, const std::vector<int64_t>& kernel_size,
    const std::vector<int64_t>& stride, const std::vector<int64_t>& padding,
    const std::vector<int64_t>& dilation, bool ceil_mode) {
    // Pooling.cpp uses stride = kernel_size before check1d.
    const std::vector<int64_t> default_stride = stride.empty() ? kernel_size : stride;
    check_dim_range(self, "max_pool1d");
    const int64_t kernel = check1d(kernel_size, "max_pool1d", "kernel_size",
                                   false, 0);
    const int64_t step = check1d(default_stride, "max_pool1d", "stride", false, 0);
...
Tensor avg_pool1d_native_cuda(
    ...
    const int64_t kernel = check1d(kernel_size, "avg_pool1d", "kernel_size",
                                   false, 0);
    const int64_t step = check1d(default_stride, "avg_pool1d", "stride", false, 0);
...
TENSORPLAY_LIBRARY_IMPL(CUDA, NativePooling) {
    m.impl("adaptive_avg_pool1d", adaptive_avg_pool1d_native_cuda);
    m.impl("adaptive_max_pool1d", adaptive_max_pool1d_native_cuda);
    m.impl("max_pool1d_with_indices", max_pool1d_with_indices_native_cuda);
    m.impl("avg_pool1d", avg_pool1d_native_cuda);
}
