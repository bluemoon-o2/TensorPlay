...
Tensor rand_like_kernel(const Tensor& self, DType dtype, Device device) {
    if (dtype == DType::Undefined) dtype = self.dtype();
    return rand_kernel(static_cast<std::vector<int64_t>>(self.shape()), dtype, device);
}

Tensor randint_like_kernel(const Tensor& self, int64_t low, int64_t high, DType dtype, Device device) {
    if (dtype == DType::Undefined) dtype = self.dtype();
    return randint_kernel(low, high, static_cast<std::vector<int64_t>>(self.shape()), dtype, device);
}

Tensor randn_like_kernel(const Tensor& self, DType dtype, Device device) {
    if (dtype == DType::Undefined) dtype = self.dtype();
    return randn_kernel(static_cast<std::vector<int64_t>>(self.shape()), dtype, device);
}

Tensor empty_like_kernel(const Tensor& self, DType dtype, Device device) {
    if (dtype == DType::Undefined) dtype = self.dtype();
    return empty_kernel(static_cast<std::vector<int64_t>>(self.shape()), dtype, device);
}

Tensor zeros_like_kernel(const Tensor& self, DType dtype, Device device) {
    if (dtype == DType::Undefined) dtype = self.dtype();
    return zeros_kernel(static_cast<std::vector<int64_t>>(self.shape()), dtype, device);
}

Tensor ones_like_kernel(const Tensor& self, DType dtype, Device device) {
    if (dtype == DType::Undefined) dtype = self.dtype();
    return ones_kernel(static_cast<std::vector<int64_t>>(self.shape()), dtype, device);
}

Tensor full_like_kernel(const Tensor& self, Scalar fill_value, DType dtype, Device device) {
    if (dtype == DType::Undefined) dtype = self.dtype();
    return full_kernel(static_cast<std::vector<int64_t>>(self.shape()), fill_value, dtype, device);
}
...
TENSORPLAY_REGISTER_KERNEL(zero_, CPU, zero_kernel) // Ensure this is compiled
...
TENSORPLAY_REGISTER_KERNEL(empty_like, CPU, empty_like_kernel)
TENSORPLAY_REGISTER_KERNEL(zeros_like, CPU, zeros_like_kernel)
TENSORPLAY_REGISTER_KERNEL(ones_like, CPU, ones_like_kernel)
TENSORPLAY_REGISTER_KERNEL(full_like, CPU, full_like_kernel)
