...
// Lightweight integer range: for (const auto i : irange(n)) { ... }
template <typename T>
class irange {
 ...
 public:
  ...
  // [begin, end) with end<=begin yields an empty range. The range-for end
  // test is `!=`, so the terminator is clamped up to begin.
