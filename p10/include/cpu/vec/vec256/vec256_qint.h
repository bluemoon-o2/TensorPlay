...
// Quantized vector registers: qint32/qint8/quint8 lanes act as efficient
// converters between the quantized storage and float registers for
// bandwidth-bound elementwise work.  Conversions:
//   Vectorized<qint8>   -> 4x Vectorized<float>
//   Vectorized<quint8>  -> 4x Vectorized<float>
//   Vectorized<qint32>  -> 1x Vectorized<float>
// Kernel code loops over float_num_vecs() when evaluating float math.
...
typename std::enable_if_t<
  ...
  // tensorplay::vec::Vectorized<float>.size() Only handle first 8*8 bits
...
template <>
struct Vectorized<tensorplay::qint32> : public Vectorizedqi {
  using size_type = int;
...
template <>
struct Vectorized<tensorplay::qint8> : public Vectorizedqi {
  ...
  using float_vec_return_type = std::array<Vectorized<float>, kFloatNumVecs>;
  using int_vec_return_type = std::array<Vectorized<tensorplay::qint32>, kIntNumVecs>;
 ...
 public:
  ...
  Vectorized(const Vectorized<tensorplay::qint8>& other) : Vectorizedqi(other.vals) {}
...
template <>
struct Vectorized<tensorplay::quint8> : public Vectorizedqi {
  static constexpr int kSize = VECTOR_WIDTH;
  static constexpr int size() {
    return kSize;
  }
 ...
 public:
  ...
  // NOLINTNEXTLINE(clang-diagnostic-deprecated-copy)
  Vectorized(const Vectorized<tensorplay::quint8>& other) : Vectorizedqi(other.vals) {}
...
struct VectorizedQuantizedConverter {
  ...
  float_vec_return_type dequantize(
    ...
    for (const auto i : tensorplay::irange(float_num_vecs())) {
      ...
      for (const auto j : tensorplay::irange(Vectorized<float>::size())) {
...
template <>
struct Vectorized<tensorplay::qint32> : public VectorizedQuantizedConverter<
                                     tensorplay::qint32,
  ...
  static Vectorized<tensorplay::qint32> loadu(const void* ptr, int64_t count) {
    ...
    std::memcpy(
        ...
        reinterpret_cast<const value_type*>(ptr),
        std::min<int64_t>(count, size()) * sizeof(value_type));
    return Vectorized<tensorplay::qint32>(tmp_values);
  }
...
template <>
struct Vectorized<tensorplay::qint8> : public VectorizedQuantizedConverter<
                                    tensorplay::qint8,
  ...
  static Vectorized<tensorplay::qint8> loadu(const void* ptr, int64_t count) {
    ...
    std::memcpy(
        ...
        reinterpret_cast<const value_type*>(ptr),
        std::min<int64_t>(count, size()) * sizeof(value_type));
    return Vectorized<tensorplay::qint8>(tmp_values);
  }
...
template <>
struct Vectorized<tensorplay::quint8> : public VectorizedQuantizedConverter<
                                     tensorplay::quint8,
                                     std::array<Vectorized<float>, 4>,
                                     std::array<Vectorized<tensorplay::qint32>, 4>,
                                     4 * Vectorized<float>::size()> {
  ...
  static Vectorized<tensorplay::quint8> loadu(const void* ptr, int64_t count) {
    ...
    std::memcpy(
        ...
        reinterpret_cast<const value_type*>(ptr),
        std::min<int64_t>(count, size()) * sizeof(value_type));
    return Vectorized<tensorplay::quint8>(tmp_values);
  }
...
std::pair<Vectorized<float>, Vectorized<float>> inline convert_int8_to_float(
    tensorplay::vec::Vectorized<int8_t> src) {
...
std::pair<Vectorized<float>, Vectorized<float>> inline convert_int8_to_float(
    tensorplay::vec::Vectorized<uint8_t> src) {
