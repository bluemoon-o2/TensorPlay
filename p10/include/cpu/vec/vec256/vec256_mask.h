...
struct VecMaskLoad<
  ...
  static inline VectorizedN<T, 2> apply(
    ...
    auto result = tensorplay::vec::VectorizedN<T, 2>();
    if constexpr (std::is_same_v<T, double>) {
      result[0] = _mm256_maskload_pd(ptr, int64_mask[0]);
      result[1] = _mm256_maskload_pd(
          ptr + tensorplay::vec::Vectorized<T>::size(), int64_mask[1]);
    } else {
      ...
      result[1] = _mm256_maskload_epi64(
          reinterpret_cast<const long long*>(
              ptr + tensorplay::vec::Vectorized<T>::size()),
          int64_mask[1]);
    }
    return result;
  }
...
#define TP_VEC_MASK_METHOD_WITH_CAST_TO_INT(                \
...
TP_VEC_MASK_METHOD_WITH_CAST_TO_INT(float, 1, bool, all_zero, (), ())
TP_VEC_MASK_METHOD_WITH_CAST_TO_INT(int64_t, 2, bool, all_zero, (), ())
TP_VEC_MASK_METHOD_WITH_CAST_TO_INT(float, 1, bool, is_masked, (int i), (i))
TP_VEC_MASK_METHOD_WITH_CAST_TO_INT(int64_t, 2, bool, is_masked, (int i), (i))
TP_VEC_MASK_METHOD_WITH_CAST_TO_INT(float, 1, bool, all_masked, (), ())
TP_VEC_MASK_METHOD_WITH_CAST_TO_INT(int64_t, 2, bool, all_masked, (), ())

#undef TP_VEC_MASK_METHOD_WITH_CAST_TO_INT
