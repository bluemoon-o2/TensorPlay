...
// Shared machinery for the 16-bit float vector layers (bfloat16 and
// half).  Registers hold packed 16-bit payloads in a 256-bit integer
// vector; every arithmetic entry converts to two float registers,
// evaluates, and converts back.
...
// bfloat16 lane conversion: widening is a 16-bit left shift; narrowing
// rounds to nearest even via the integer-bias trick and re-injects the
// canonical quiet NaN pattern for NaN inputs.
...
static inline __m128i cvtfp32_bf16(const __m256& src) {
  ...
  t_value = _mm256_packus_epi32(t_value, t_value);
  t_value = _mm256_permute4x64_epi64(t_value, 0xd8);
...
// half lane conversion: F16C covers both directions with the hardware
...
template <>
inline void cvt_to_fp32<tensorplay::BFloat16>(const __m128i& a, __m256& o) {
  cvtbf16_fp32(a, o);
}
template <>
inline void cvt_to_fp32<tensorplay::Half>(const __m128i& a, __m256& o) {
  cvtfp16_fp32(a, o);
}
...
template <>
inline void cvt_to_fp32<tensorplay::BFloat16>(const __m256i& a, __m256& o1, __m256& o2) {
  cvtbf16_fp32(a, o1, o2);
}
template <>
inline void cvt_to_fp32<tensorplay::Half>(const __m256i& a, __m256& o1, __m256& o2) {
...
template <>
inline __m256i cvt_from_fp32<tensorplay::BFloat16, false>(
    const __m256& a,
    const __m256& b) {
  return cvtfp32_bf16(a, b);
}
template <>
inline __m256i cvt_from_fp32<tensorplay::BFloat16, true>(
    const __m256& a,
    const __m256& b) {
  return merge_compare_result(a, b);
}
template <>
inline __m256i cvt_from_fp32<tensorplay::Half, false>(
    const __m256& a,
    const __m256& b) {
  return cvtfp32_fp16(a, b);
}
template <>
inline __m256i cvt_from_fp32<tensorplay::Half, true>(
    const __m256& a,
    const __m256& b) {
  return cvtfp32_fp16(a, b);
}
...
class Vectorized16 {
 ...
 public:
  ...
  static Vectorized<T> loadu(const void* ptr, int64_t count = size()) {
    if (count == size())
      return _mm256_loadu_si256(reinterpret_cast<const __m256i*>(ptr));

    // The contract is 0 <= count <= size(); clamp so the memcpy bound is
    // provable and out-of-contract callers stay safe.
    const int n = static_cast<int>(std::clamp<int64_t>(count, 0, size()));
    // Zero tail past `n`.
  ...
  void store(void* ptr, int64_t count = size()) const {
    if (count == size()) {
      ...
      const int n = static_cast<int>(std::min<int64_t>(count, size()));
  ...
  Vectorized<T> fmod(const Vectorized<T>& q) const {
    ...
    _mm256_storeu_ps(tx, x_lo);
    _mm256_storeu_ps(tx + 8, x_hi);
    _mm256_storeu_ps(tq, q_lo);
    _mm256_storeu_ps(tq + 8, q_hi);
    for (int64_t i = 0; i < size() / 2; i++) {
      tx[i] = std::fmod(tx[i], tq[i]);
      tx[i + 8] = std::fmod(tx[i + 8], tq[i + 8]);
    }
    auto o1 = _mm256_loadu_ps(tx);
    auto o2 = _mm256_loadu_ps(tx + 8);
    return cvt_from_fp32<T>(o1, o2);
  }
...
#define TP_CONVERT_VECTORIZED_INIT(type, name)                  \
...
#define TP_LOAD_FP32_VECTORIZED_INIT(type, name)                            \
