...
// AVX-512 masked-load and mask-cast specializations of the VecMask
// customization points (float/int32/64-bit pairs and packed 16-bit loads).
...
struct VecMaskLoad<
  ...
  static inline VectorizedN<T, dst_n> apply(
    ...
    tensorplay::vec::Vectorized<T> zero_vec(0);
...
struct VecMaskLoad<
    ...
    dst_n,
    std::enable_if_t<
        std::is_same_v<data_t, tensorplay::BFloat16> || std::is_same_v<data_t, tensorplay::Half>>> {
...
struct VecMaskLoad<
  ...
  static inline VectorizedN<data_t, 2> apply(
      ...
      const VecMask<mask_t, 1>& vec_mask) {
    auto all_ones = _mm512_set1_epi32(0xFFFFFFFF);
    tensorplay::vec::Vectorized<data_t> zero_vec(0);
    auto int_mask = vec_mask.template cast<int, 1>()[0];
    auto mmask = _mm512_cmp_epi32_mask(int_mask, all_ones, _MM_CMPINT_EQ);
    tensorplay::vec::VectorizedN<data_t, 2> result;
