...
// Float8 (e4m3fn / e5m2) 512-bit registers: 64 packed 8-bit lanes acting as
// converters between the fp8 storage and four float registers.  Lane
// conversion runs on plain AVX512 integer sequences (renorm via leading-zero
// counting, saturating narrowing with round-to-nearest-even); the AVX10.2
// native convert instructions take over when the target defines them.
...
static inline void cvtfp8e4m3_fp32(const __m128i& a, __m512& o) {
  ...
  if (denormal_mask) {
    ...
    // Leading-zero count of the 3-bit mantissa, then the 5 exponent zeros,
...
class Vectorizedf8 {
  static_assert(
      std::is_same_v<T, tensorplay::Float8_e4m3fn> ||
          std::is_same_v<T, tensorplay::Float8_e5m2>,
      "Support only float8 e4m3/e5m2.");
 ...
 private:
  ...
  Vectorized<T> inline binary_compare(const VectorizedType& b, Op op) const {
    ...
    __m512 b0, b1, b2, b3;
    __m512 o0, o1, o2, o3;
    if constexpr (std::is_same_v<T, tensorplay::Float8_e4m3fn>) {
...
// FP8 arithmetic exists so generic templates compile; kernels evaluate
// through the float registers.
