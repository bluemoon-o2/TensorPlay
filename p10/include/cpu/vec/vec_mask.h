...
// `VecMask` wraps a `VectorizedN<T, N>` of lane masks (all-ones = active)
// and offers boolean conversion, cross-type casting, masked loads and
// whole-mask predicates.  A small set of customization points
// (`VecMaskLoad` / `VecMaskTo` / `VecMaskCast` / `VecMaskCheck`) lets the
// per-ISA layers swap generic scalar implementations for intrinsic ones.
...
class VecMask {
 ...
 public:
  ...
  void store(bool* b, int count = size()) {
    ...
    auto res = this->template to<bool, L>();
    res.store(b, count);
    return;
  }
...
#define TP_VEC_MASK_DEFINE_UNARY_OP_GLOBAL(op)      \
  template <typename T, int N>                      \
  inline VecMask<T, N> op(const VecMask<T, N>& a) { \
    return op(VectorizedN<T, N>(a));                \
  }

#define TP_VEC_MASK_DEFINE_BINARY_OP_GLOBAL(op)                               \
  template <                                                                  \
      typename T,                                                             \
      int N,                                                                  \
      typename V,                                                             \
...
#define TP_VEC_MASK_DEFINE_BINARY_OP_WITH_EXPR_GLOBAL(op, EXPR)               \
  template <                                                                  \
      typename T,                                                             \
      int N,                                                                  \
      typename V,                                                             \
...
TP_VEC_MASK_DEFINE_UNARY_OP_GLOBAL(operator~)
TP_VEC_MASK_DEFINE_BINARY_OP_GLOBAL(operator&)
TP_VEC_MASK_DEFINE_BINARY_OP_GLOBAL(operator|)
TP_VEC_MASK_DEFINE_BINARY_OP_GLOBAL(operator^)
TP_VEC_MASK_DEFINE_BINARY_OP_GLOBAL(operator*)
TP_VEC_MASK_DEFINE_BINARY_OP_WITH_EXPR_GLOBAL(operator>, a & ~b)
TP_VEC_MASK_DEFINE_BINARY_OP_WITH_EXPR_GLOBAL(operator<, ~a& b)
TP_VEC_MASK_DEFINE_BINARY_OP_WITH_EXPR_GLOBAL(operator==, ~(a ^ b))
TP_VEC_MASK_DEFINE_BINARY_OP_WITH_EXPR_GLOBAL(operator>=, (a == b) | (a > b))
TP_VEC_MASK_DEFINE_BINARY_OP_WITH_EXPR_GLOBAL(operator<=, (a == b) | (a < b))
TP_VEC_MASK_DEFINE_BINARY_OP_WITH_EXPR_GLOBAL(operator!=, (a ^ b))

#undef TP_VEC_MASK_DEFINE_UNARY_OP_GLOBAL
#undef TP_VEC_MASK_DEFINE_BINARY_OP_GLOBAL
#undef TP_VEC_MASK_DEFINE_BINARY_OP_WITH_EXPR_GLOBAL
